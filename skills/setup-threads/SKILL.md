...
description: >
  ...
  setup, and OAuth token issuance through the ego lite browser with HITL handoffs
  (login·verification code·consent), then saves the 60-day token to
...
# Threads 채널 개설 — ego lite HITL

채널(브랜드) 하나에 **살아 있는 Threads 계정을 붙이는** 스킬이다. 계정 개설부터
프로필 브랜딩, Meta 앱 권한 설정, OAuth 토큰 발급까지를 ego lite 브라우저로
...
ego lite 로 폼 입력·페이지 이동·동의 클릭까지는 자동으로 한다. 다음은 **자동화가
...
각 핸드오프 앞에서 `handOffTaskSpace` 로 화면을 사용자에게 노출하고, 사용자
완료를 확인한 뒤 `takeOverTaskSpace` 로 회수해 이어 간다.

## 전제 조건

- `data/<slug>/profile.md` 가 있어야 한다 — 없으면 `/social-flow:channel add` 부터
  안내하고 중단한다. 핸들·브랜드 톤은 여기서 상속한다.
- **ego lite**(`ego-browser` CLI)가 이 머신에 있어야 한다 — 이 플러그인의 구성물이
  아니라 사용자 머신의 브라우저 자동화 도구다. 없으면 §수동 폴백으로 진행한다.
...
ego lite 가 없으면 개설·브랜딩은 사용자가 브라우저에서 직접 하고, 이 스킬은
