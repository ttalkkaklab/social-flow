...
문안마다 원 댓글과 그 댓글이 달린 우리 영상의 제목·설명을 동봉한다(이것 없이는
맥락 축이 0점이다). `growth-plan.md`·`profile.md` 경로와 자가 검사 exit code 도
싣는다. **score ≥95 이고 p0=0 인 문안만 보낸다.** FAIL 은 교정 지시대로 빼기만
해서 고친다 — 없던 비유·상투구를 새로 심으면 그게 새 AI 티다. 최대 3라운드,
미달이면 그 답글은 보내지 않고 growth-log 에 `스킵(게이트 NN점)` 으로 적는다.
