...
description: >
  ...
  complete. Takes the finished per-channel content under data/<category>/<topic>/output/
  through account checks, public media hosting, a mandatory HITL approval gate, and
  publishes via the social-flow MCP channel tools (threads/instagram/facebook/
...
# 채널 게시 — HITL 승인 후 즉시 공개

`output/` 의 완성 산출물을 채널별 게시 툴로 게시한다. **게시 툴은 검토 게이트가
...
1. **게시 전 사용자 승인 필수 (HITL)** — 호출 전 반드시 AskUserQuestion 으로
   채널별 최종본 전문을 제시하고 명시 승인을 받는다. IG 는 게시 후 이미지 교체
   불가, 전 채널 브랜드 계정에 박제된다(비가역).
2. **크로스포스팅 복붙 금지** — 같은 문장이 두 채널에 그대로 들어가면 안 된다.
3. **토큰 평문 노출 금지** — 토큰은 `~/.config/social-flow/` 파일로만 다룬다.
   curl 디버깅 시에도 `$(cat …)` 인라인 참조만, 토큰 값·access_token 필드를
   대화에 출력하지 않는다.
4. **사실 왜곡 금지** — 승인된 산출물 범위 안에서만 게시. 캡션을 즉석에서 고쳐야
   하면 수정본으로 승인 게이트를 다시 거친다.
5. **한도 존중** — Threads 250건/24h, IG 100건/24h, YouTube 업로드 일 6건.
6. **링크는 본문이 아니라 댓글/답글로** — Threads 는 게시 성공 직후 응답 postId 를
   `replyToId` 로 넣은 링크 답글, FB 는 `facebook_comment` 첫 댓글.
   **링크 댓글까지 게시돼야 그 채널의 게시가 완료된 것이다.**
...
- `sns_account_check` 로 채널별 토큰 상태 점검 — **게시 계정이 어디인지 이 결과로
  확인·보고한다**(계정은 설정이 아니라 토큰 /me 로 결정). 무효 채널은 대상에서
  제외하고 사유를 보고. 게시 툴이 ListTools 에 아예 없으면 토큰 파일 미설정이다 —
  `references/token-setup.md` 절차를 안내한다. Meta 계열 토큰은 60일 갱신형 —
  파일 나이 45일 초과 시 갱신을 권고한다.

### 1. HITL 승인 게이트 (호스팅보다 먼저 — 미승인 콘텐츠를 공개 URL 에 올리지 않는다)

AskUserQuestion 으로 채널별 최종본을 제시한다 — 본문/캡션 전문, 미디어 **로컬
경로**(cover.jpg·video.mp4), 해시태그, Threads 답글·FB 첫 댓글 문안까지 전부,
게시 예정 계정(계정 점검 결과). 선택지: [전체 게시 / 일부 채널만 / 수정 후
...
링크 대체(YouTube permalink 등)를 사용자에게 묻는다. 나머지 채널 순서는 자유.
...
- `data/<카테고리>/<주제>/output/publish-log.md` 에 기록: 일시·채널·게시 id·
  permalink·캡션 요약·승인자 결정 표.
- `storyboard.md` `status: published` 갱신.
- 임시 터널을 썼으면 §1 철거 검증 후, 채널·permalink 표로 최종 보고한다.

## Additional Resources

### Reference Files

- **`references/token-setup.md`** — 채널별 자격증명 발급·파일 규약·갱신 절차 (Threads/IG 60일 · FB 무기한 · YouTube OAuth)
