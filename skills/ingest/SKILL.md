...
- **start 가 찍는 `입력 장치 / 입력 볼륨` 줄을 확인하고 넘어간다** — `-g` 는 그
  시점의 기본 입력 장치를 캡처한다. 외장 마이크를 꽂아도 macOS 기본 입력은 자동으로
  바뀌지 않으므로, 엉뚱한 마이크나 너무 낮은 볼륨이면 녹화가 끝난 뒤에야 안다.
  고정하려면 `SF_MIC_DEVICE`(장치 이름)·`SF_MIC_VOLUME`(0~100) 환경변수를 주고 시작한다.
