...
PATTERNS: list[Pattern] = [
    ...
    # 는 누가 누구에게 요구하는지가 없다). `(?![면고])` 는 D8 과 같은 갈래 가드다 —
    # 조건절("요구된다면")·연결형("요구된다고 한다")은 종결이 아니다. 관형형도 빠진다.
    ("T12", "S2", "무주어 피동 ~이 요구된다",
     re.compile(r"[이가]\s*요구(된다|됩니다|돼요|되었다|되었습니다|됐다|됐습니다|되고\s*있(다|습니다))(?![면고])"), 0,
    ...
    ("T13", "S3", "무생물 주어 능동",
     re.compile(r"(결과|수치|통계|자료|데이터|보고서|기록|지표)[은는이가]\s[^.!?\n]{0,60}"
                r"(말해\s*(준다|줍니다|주고\s*있(다|습니다))|웅변(한다|합니다))(?![면고])"), 0,
...
SELFTEST = [
    ...
    ("T11 정상 사동 오탐 없음", "fb", 0,
     "아이를 학원에 등록시켰어요.\n차를 갓길에 정지시켰어요.\n매출을 크게 증가시켰어요.\n"),
    # T12 는 종결형만 본다 — 관형형·조건절·연결형이 걸리면 세 건이라 79점이 된다.
    ("T12 관형·조건·연결형 오탐 없음", "fb", 0,
     "요구되는 서류가 늘었어요.\n주의가 요구된다면 그때 공지할게요.\n"
     "추가 서류가 요구된다고 해요.\n"),
    # T13 도 종결형만 — S3(2점)이라 exit 으로는 안 갈리지만 픽스처는 남긴다.
    ("T13 관형·연결형 오탐 없음", "fb", 0,
