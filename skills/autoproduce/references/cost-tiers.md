...
올릴 수 있는 것은 **커버 다음 구간의 b-roll 하나뿐**이다. `veo_img2video` 로
커버 배경 PNG 를 움직인다.

```
모델 veo-3.1-fast-generate-preview · 1080p · 8초 생성 · 9:16
...
예: "very slow push-in, nearly static camera, subtle particle drift.
...
**승급 조건은 관측값이다.** 성장 루프가 이미 읽고 있는 지표를 그대로 쓴다 —
셋 중 하나라도 참이면 다음 한 편만 승급한다.

| 플랫폼 | 지표 | 승급 임계 |
|---|---|---|
| YouTube | `averageViewPercentage` (최근 3편 평균) | 30% 미만 |
| Instagram | `reels_skip_rate` (최근 3편 평균) | 0.55 초과 |
| 공통 | 게시 이력 3편 미만 | 해당 없음 — **승급하지 않는다** |

지표가 3편치 모이기 전에는 경제 기본으로만 돌린다. 표본 없이 올리면 그냥
비싼 기본값이 된다. 승급은 **한 편 단위**다 — 조건이 계속 참이면 다음 편도
승급하되 자동으로 "승급 모드"에 고정되지는 않는다.
...
막는다** — fast·1080p·8초 승급은 편당 약 $1.23(경제 기본 + $0.96)이라 상한을
**$1.30 이상**으로 올려야 지나간다. 상한 인상은 무인 루프가 결정하지 않는다 —
...
$REF/cost-report.sh .work/cost-tally.tsv --cap 0.30; echo "cost_exit=$?"
```
...
현재 미검증으로 남은 것: **Veo 초당 단가 전부**(공식 요금 페이지에서 나왔지만
교차 확인을 안 거쳤다)와 **Lyria 클립 단가**. 실과금 호출로 청구서를 한 번
받아 보면 등급을 올릴 수 있다.
