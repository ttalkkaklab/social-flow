...
| `hooking` | 후킹 | 문제·피해·결심. 왜 그 결과가 필요한지 — 커버가 던진 것을 받고, 답은 안 푼다 | **커버 바로 다음, 모든 회차에** (§hooking) |
...
- **하는 일 넷** — 자료가 겹치는 자리다(근거는 [후킹 리서치](../../../docs/research/2026-08-18-hooking-beat/)).
  1. **받는다** — 커버가 던진 것과 같은 대상, 같은 약속을 이어 간다. 커버에 없던 소재로
     ...
     맞았는가.
  2. **건다** — 시청자의 문제·손해·이익을 **시청자 주어**로 말한다. 형태는 셋 중 하나:

     | 형태 | 예 |
     |---|---|
     | 문제·피해 (공감형) | "이 동네 첫 카페 개업이야! … 순식간에 카페거리가 돼버렸네." |
     | 미해결 긴장 (호기심형) | "반응이 없을 것 같은 영상이면, 너희들 결정은 뭐야?" |
     | 결심·기준 (선언형) | "주변 상가 오픈 정보를 미리 알 수 있게 서비스를 만들어야겠어." |
