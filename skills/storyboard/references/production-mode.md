...
## Six render ratios and portal allocation (2026-09-22)

The machine-readable band table is [render-ratios.json](render-ratios.json).

New selections write `PRODUCTION.renderRatioVersion: 1` alongside `mode`. Count
**all SCENES entries**, by shot count, with `generated_video` and `stock_video` both
counting as video. This is a rendering ratio; the paid-generation cost denominator
is separate and still excludes supplied footage and reuse.

| Mode | Label | Video share |
|---|---|---|
| `full_video` | 전체 영상 | 100% |
| `video_80` | 80% 이상 영상 | [80%, 100%) |
| `video_50` | 50% 이상 영상 | [50%, 80%) |
| `video_30` | 30% 이상 영상 | [30%, 50%) |
| `video_lt10` | 10% 미만 영상 | [0%, 10%) |
| `no_video` | 0% 영상 | 0 shots |

Aim at `ceil(N × lower percentage)` video shots; any integer inside the band passes.
The strict upper count is `ceil(N × upper percentage) - 1`. If that is below the
minimum, use the minimum for both bounds (1 shot at 80% → 1; 2 shots at 30% → 1).
At 10 shots, less than 10% allows zero; at 11, it allows one. No duration weighting.
`production-mode.js` exposes `ratioBounds` and enforces this through `check` and `policy`.
Do not relabel a legacy approval: without the version marker, `video_50` and
`video_30` keep their former minimum-only generated-cut contract, and `hook_only`
keeps its original hook. On an explicit new selection, `hook_only` maps to
`video_lt10`, `stills_only` to `no_video`; `hybrid` stays legacy only.

With a portal key, call `portal_render_allocation` with the episode ID or directory
before local authoring. If its request is pending, the host LLM reads every returned
shot, chooses its render route and writes a specific purpose and reason for each ID.
Prioritize essential continuous action for video. Hooks are candidates, not automatic
paid footage; use data_graph for quantities, object_html for mechanisms, and
still_camera for people or locations where camera motion suffices. Preserve supplied
footage; never invent a stock source or license to satisfy a percentage.

Submit all assignments in one `portal_render_allocation` call, with the exact
`requestId` and `baseRevisionNo` from the read. The portal rejects duplicate/missing
IDs, out-of-band counts, stale revisions and foreign leases before saving. A conflict
requires a fresh read and reasoning against those shots. Then `portal_storyboard_pull`
the saved revision before editing locally. Finish the render-specific visual handoffs
and run `storyboard_check`: changing mode alone does not make old assets suitable.
A changed ratio invalidates the old cost approval; quote the current generation plan
and follow the existing approval rules before spending. No asset is generated by this tool.

The portal records **배분 대기** and the request time; a running host session consumes
it. Selecting the UI option does not start a background LLM worker.
...
After capability and format checks, before visual planning or paid assets, present **all six**
...
Use the six modes in the table above. Legacy-only modes are not new choices.
...
is not one of the six choices and carries no cost approval. Use it only while the channel cap
...
**Legacy approvals without renderRatioVersion:** count new cuts, not duration; round the minimum up. Exclude imported `visual.reuse` clips,
...
> - 전체 영상 — 새 컷을 전부 생성 영상으로 만듭니다. 여섯 옵션 중 생성할 컷이 가장 많습니다. [영상 수·모델·해상도·음성 여부]. 최초 [금액], [횟수]회 시도 [범위]. 영상 예산 상한 [금액].
...
> - 80% 이상 영상 — 영상 비율 80% 이상으로 배분합니다. [영상 수·모델·해상도·음성 여부·비용·상한].
> - 10% 미만 영상 — 영상 비율을 10% 미만으로 제한합니다. [영상 수·모델·해상도·음성 여부·비용·상한].
> - 0% 영상 — 모든 샷을 정지컷·HTML 설명으로 구성합니다. 영상 생성비 0, 다른 제작비는 별도입니다.
...
All six choices keep the selected `style`; hybrid keeps purpose-based render routing.
...
page shows all six choices, retries and the selected cap, and flags a stale comparison.
