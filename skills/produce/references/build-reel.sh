...
while IFS=$'\t' read -r -u 3 IDX SRC TARGET ZDIR OPTS; do
  ...
  if [ -n "${OPTS:-}" ]; then
    ...
    for KV in "${OARR[@]}"; do
      ...
      case "$KV" in
        ...
        # a moving join starts the next line at this card's first frame (§4.5).
        # `enter=` is the join in front of this card, `exit=` the one behind it. The edit compiler
        # writes both halves of a dip and reserves the outgoing live handle for a moving join;
        # an explicit value here only has to agree with the compiled plan.
  ...
  # A filmed card (sync) gets no margins — any pre-roll would start the picture at 0s while the sound alone lags.
