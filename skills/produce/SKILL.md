...
---
**Portal HITL answers:** G12 assembly warnings → assembly_warning:ID; G13 failed slide rewrites → slide_fallback:ID. Record the actual user choice and warning evidence even when the answer arrives only in conversation. Follow [the decision contract](../storyboard/references/portal-decisions.md): save each answer immediately, then let the next existing checkpoint carry `decisions[]`. Never infer approval from completion.
...
## Absolute rules
Read [production-mode.md](../storyboard/references/production-mode.md) at entry: present 100% 이상, 50% 이상, 30% 이상 and 훅만 영상 choices **with first-pass and retry-inclusive costs and the budget cap**, then persist the user's selection. Reuse an existing approval on resume. Run `check-production.js storyboard/ --selection` before assets.
