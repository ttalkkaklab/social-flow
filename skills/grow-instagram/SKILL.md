...
도달 상위 릴스의 유형(소재·길이·형식)과 미디어별 `follows`(팔로우 전환)를 읽어
관찰 결과를 사용자에게 보고한다 — 이 학습 루프가 없으면 자동화는 같은 영상만
반복한다. 기획 반영은 storyboard 파이프라인에서 사람이 한다.
...
넘긴다 — 최근 릴스 3편의 `reels_skip_rate` 평균이 0.55 를 넘으면 훅이 죽고
있다는 뜻이라 도입 b-roll 로 승급하고, 게시 이력이 3편 미만이면 승급하지
않는다(`references/cost-tiers.md`).
