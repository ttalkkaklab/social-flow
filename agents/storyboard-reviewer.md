...
- **훅·전달 (30)**: 커버 훅에 긴장이 있고 주제어가 보임 — **첫 프레임·제목·첫 대사
  ...
  0 / 쉬운 말 — 처음 듣는 사람이 따라옴 10 / 한 회차 한 결과로 씬 전개가
  이어지고, 제작형이면 결과물이 내용보다 앞에 있어 중간 이탈 지점이 없음 10
...
| points | 한 화면에 한 메시지. `beat:"hooking"` 이면 문제·피해·결심, `beat:"result"` 면 완성본을 펼치고, `beat:"body"` 면 그 결과를 만든 방법만 말한다. 캡션이 제목을 되풀이하지 않는다 |
