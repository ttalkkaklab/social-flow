...
- 육성 씬(`narration: []`)인데 대사가 있거나, 나레이션 덮기 씬인데 비어 있는가.
  둘 다 소리가 겹치거나 비는 결함이다(P0). **단 전 씬 육성 회차
  (`window.VOICE === "user"`)는 이 잣대를 쓰지 않는다** — 그 회차는 전 씬이
  narration 을 채우는 것이 계약이고(scenes-schema §전 씬 육성 회차), 촬영 씬의
  세그도 TTS 대본이 아니라 말할 문장이다. 이때는 반대로 **비어 있는 씬**이 결함이다.

**슬라이드 씬(`visual.slide`)에서 보는 것도 세 가지다** (scenes-schema §슬라이드 씬).
- `plan` 이 **그 씬의 나레이션을 실제로 그림으로 만드는 계획인가.** 나레이션이 구조를
  말하는데 plan 이 분위기 컷이면 그 씬은 슬라이드일 이유가 없다.
- `labels` 가 **완결적인가.** 나레이션이 이름을 부르는 구조 요소(폴더·단계·항목)가
  labels 나 title·bullets 어디에도 없으면, 화면이 소리를 못 받친다.
- 슬라이드 파일(`slides/*.html`) 부재는 **결함이 아니다** — 파일은 승인 뒤
  storyboard §8 에서 나온다. 여기서 보는 것은 계획이지 산출물이 아니다.
...
**슬라이드 씬(`visual.slide`)은 이 모드의 대상이 아니다** — 그 씬의 화면은 생성
이미지가 아니라 승인 뒤 storyboard §8 이 만드는 HTML 슬라이드라, `scene-N.png` 가
없는 것이 정상이다. 부재를 결함으로 올리지 않는다.
