...
- **훅·전달 (30)**: 커버 훅에 긴장이 있고 주제어가 보임 — **제목과 첫 대사 세그①을
  각각 본다**(제목 5 + 세그①이 약속·문제 제기·반전 형태 5) / 쉬운 말 — 처음 듣는
...
**본편 씬 전부**를 채점한다(cover·points·quote·broll). `outro` 는 공용 자산이라 제외.

## 타입별 역할 기준 (품질 축의 잣대)

| 타입 | 그 씬이 해야 하는 일 |
|---|---|
| cover | 3초 안에 **무엇의 이야기인지** 알리고 긴장을 만든다 |
| points | 한 화면에 한 메시지. 캡션이 제목을 되풀이하지 않는다 |
...
- **품질 (50)**: 타입 역할 수행 30 / 표면 분업 — title 은 구어 훅, 캡션은 정보 한 줄,
  나레이션은 존댓말 설명으로 각자 다른 일을 함 10 / 길이 대비 정보 밀도 10
