...
export async function webSearch(input: WebSearchInput): Promise<SerpResult> {
  ...
  const slim = compact({
    ...
    note: sliceNote(
      rawOrganic.length,
      limit,
      `page 를 1씩 올리면 전역 ${offset + GOOGLE_PAGE_SIZE + 1}번부터 이어지므로 그 사이 구간은 건너뛴다 — 빠짐없이 보려면 limit 을 ${GOOGLE_PAGE_SIZE} 이하로 두고 page 를 차례로 넘길 것`,
    ),
    ...
    text: JSON.stringify(
      compact({
        ...
        note: sliceNote(
          rawImages.length,
          max,
          `이 엔진은 한 번에 ${GOOGLE_IMAGES_PAGE_SIZE}건을 주는데 limit 상한이 ${SERP_IMAGE_MAX_LIMIT} 라 그 이상은 받을 수 없다. page 한 칸이 ${GOOGLE_IMAGES_PAGE_SIZE}건이라 잘린 구간은 page 로도 닿지 않는다 — 필터(size·aspect·license)로 좁혀 재조회할 것`,
        ),
        images,
