...
export async function webSearch(input: WebSearchInput): Promise<SerpResult> {
  ...
  const organicResults = ((p.organic_results as any[]) ?? []).slice(0, num).map((r) =>
    compact({
      position: r.position,
      title: r.title,
      link: r.link,
...
export async function newsSearch(input: NewsSearchInput): Promise<SerpResult> {
  ...
  const p = json as Record<string, any>;
  const newsResults = ((p.news_results as any[]) ?? []).slice(0, max).map((r) =>
    compact({
      position: r.position,
      title: r.title,
 ...
 */
function naverSortValue(where: 'web' | 'news', sortBy?: 'relevance' | 'latest'): string | undefined {
  if (!sortBy) return undefined;
  if (where === 'news') return sortBy === 'latest' ? '1' : '0';
...
export async function naverSearch(input: NaverSearchInput): Promise<SerpResult> {
  ...
  const items = slimItems(
    (p.organic_results as any[]) ?? (p.web_results as any[]) ?? (p.news_results as any[]),
  );
  if (items.length === 0) {
    return {
      text: '(검색 결과 없음 — 검색어를 바꿔 1회만 재시도하거나, 확인 실패한 주장은 본문에서 제외할 것)',
