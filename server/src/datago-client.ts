...
// ── 1. Dataset search (no auth, HTML parsing) ─────────────────────
...
async function searchOneType(keyword: string, type: DatagoType, page: number, perPage: number) {
  ...
  if (!res.ok) return { error: `data.go.kr search HTTP ${res.status}: ${res.body.slice(0, 200)}` };
...
export async function searchDatasets(input: DatagoSearchInput): Promise<DatagoResult> {
  ...
  if (empty) {
    return {
      text:
        '(no search results — retry with a different query. If it stays at 0, a portal redesign may have broken parsing: ' +
...
// ── 2. Dataset detail (no auth, HTML parsing) ─────────────────────
...
export async function datasetDetail(input: DatagoDetailInput): Promise<DatagoResult> {
  ...
  const res = await requestRaw('get', url, { 'User-Agent': BROWSER_UA }, undefined, 20_000);
  if (!res.ok) return err(`data.go.kr detail HTTP ${res.status} — check that publicDataPk(${input.publicDataPk}) and type match the search result`);
  const html = res.body;
...
// ── 3. Raw file-data download (no auth, two steps) ────────────────
...
export async function downloadFile(input: DatagoDownloadInput): Promise<DatagoResult> {
  ...
  if (!step1.ok) return err(`download step 1 HTTP ${step1.status}: ${step1.body.slice(0, 200)}`);
  ...
  if (!fileRes.ok) return err(`download step 2 HTTP ${fileRes.status}`);
  ...
  if (buf.length > MAX_DOWNLOAD_BYTES) return err('the file is over the 100MB cap — query only the rows you need with datago_file_fetch.');
...
// ── 4. File-data row query — odcloud API (API key required) ───────
...
// ── 5. Standard open-API call — apis.data.go.kr (API key required) ─
...
const OPENAPI_AUTH_HELP =
  'auth refused — this key is not registered for this API. Check on data.go.kr that the usage application (활용신청, mostly auto-approved and instant) for that API is in place, ' +
...
export async function callOpenApi(input: DatagoApiCallInput): Promise<DatagoResult> {
  ...
  if (path.includes('..') || path.includes('://')) return err('path only accepts a route under apis.data.go.kr (e.g. 1360000/VilageFcstInfoService_2.0/getUltraSrtNcst)');
