...
export const normalizeSpeech = (s: string): string => s.normalize('NFKC').toLowerCase().replace(/[\p{P}\p{Z}\s]/gu, '');
...
export function characterErrorRate(expected: string, heard: string): number {
...
export async function measureSignal(file: string): Promise<Signal> {
  ...
  for (let i = 0; i < n; i++) {
    ...
    sum += x * x; if (Math.abs(x) >= 0.999) clipped++;
  }
...
export function reviewFailures(expected: string, transcript: string, review: Review, duration: number): string[] {
  const failures = [];
  ...
  for (const axis of ['accuracy', 'pronunciation', 'naturalness', 'clarity'] as const) {
  ...
  if (review.issues.some(i => i.end < i.start || i.end > duration + 0.1)) failures.push('Invalid issue timestamps');
  return failures;
}
...
export async function listen(file: string, request: CheckedSpeechRequest): Promise<{ transcript: string; review: Review }> {
  ...
  async function call(prompt: string, schema: Record<string, unknown>, stage: string): Promise<unknown> {
    ...
    try {
      ...
      const result: unknown = JSON.parse(response.text || ''); ok = true; return result;
    } finally {
  ...
  const blind = z.object({ transcript: z.string().min(1).max(12000) }).strict().parse(await call(
...
export async function generateCheckedSpeech(input: CheckedSpeechRequest, dependencies?: QualityDependencies): Promise<Record<string, unknown>> {
  ...
  catch { return { success: false, status: 'unverified', error: 'This audio has an active review lock. Wait for it; after a crashed process, inspect the lock before removing it.' }; }
  ...
  try {
    ...
    closeSync(lock); rmSync(lockFile, { force: true });
  }
}
