...
export interface ScoutedKeyword {
  phrase: string;
  score: number;
  outlierCount: number;
  bestMultiplier: number;
  evidence: Array<{ videoId: string; title: string; multiplier: number }>;
}

export interface ScoutedChannel {
  channelId: string;
  title: string;
...
function parseJson(body: string): Raw | null {
  try {
...
export function scoreKeywords(
  outliers: Array<{ videoId: string; title: string; multiplier: number; tags?: string[] }>,
  limit: number,
): ScoutedKeyword[] {
  const byPhrase = new Map<
    string,
    { score: number; count: number; best: number; evidence: ScoutedKeyword['evidence'] }
  >();

  const add = (phrase: string, video: { videoId: string; title: string; multiplier: number }, weight: number) => {
    const cur = byPhrase.get(phrase) ?? { score: 0, count: 0, best: 0, evidence: [] };
    cur.score += video.multiplier * weight;
    cur.count += 1;
    if (video.multiplier > cur.best) cur.best = video.multiplier;
  ...
  for (const video of outliers) {
    ...
    for (const tag of video.tags ?? []) {
      ...
      if (cleaned) add(cleaned, video, 0.4);
    }
  }
...
function isShortEnough(seconds: number | null, duration: 'short' | 'any'): boolean {
  if (duration === 'any') return true;
  if (seconds == null) return true;
  return seconds <= 180;
}

async function youtubeGet(path: string, params: Record<string, string>, auth: Auth): Promise<ApiResult> {
...
async function resolveAuth(channel?: string): Promise<{ auth?: Auth; error?: ApiResult; via: string }> {
  if (config.youtubeApiKey) return { auth: { kind: 'key', key: config.youtubeApiKey }, via: 'api_key' };
  ...
  if (!token) return { via: 'oauth', error: tokenError };
  return { auth: { kind: 'bearer', token }, via: 'oauth' };
}

async function ownChannelId(auth: Auth): Promise<string | undefined> {
  if (auth.kind !== 'bearer') return undefined;
  const res = await youtubeGet('channels', { part: 'id', mine: 'true' }, auth);
  if (!res.ok) return undefined;
  const id = str(((parseJson(res.body)?.items as Raw[] | undefined)?.[0] ?? {}).id);
  return id || undefined;
}
...
async function searchChannels(
  queries: string[],
  auth: Auth,
  opts: { regionCode: string; language: string; publishedAfter: string; duration: 'short' | 'any' },
): Promise<{ hits: SearchHit[]; units: number; errors: string[] }> {
  ...
  for (const query of queries) {
    ...
    if (opts.duration === 'short') params.videoDuration = 'short';
    ...
    for (const item of (parseJson(res.body)?.items as Raw[] | undefined) ?? []) {
...
async function loadChannelUploads(
  channelIds: string[],
  auth: Auth,
  videosPerChannel: number,
): Promise<{
  byChannel: Map<string, { title: string; subscriberCount: number | null; uploads: ChannelUpload[] }>;
  units: number;
  errors: string[];
}> {
  const byChannel = new Map<string, { title: string; subscriberCount: number | null; uploads: ChannelUpload[] }>();
  const errors: string[] = [];
  let units = 0;

  const playlistByChannel = new Map<string, string>();
  for (const group of chunk(channelIds, 50)) {
    ...
    for (const item of (parseJson(res.body)?.items as Raw[] | undefined) ?? []) {
      ...
      const snippet = (item.snippet as Raw | undefined) ?? {};
      const stats = (item.statistics as Raw | undefined) ?? {};
      const uploads = str(
        ((item.contentDetails as Raw | undefined)?.relatedPlaylists as Raw | undefined)?.uploads,
      );
      ...
      if (uploads) playlistByChannel.set(channelId, uploads);
    }
  }

  const videoIds: string[] = [];
  const ownerByVideo = new Map<string, string>();
  const playlistEntries = [...playlistByChannel.entries()];
  for (const group of chunk(playlistEntries, 5)) {
    const results = await Promise.all(
      group.map(([, playlistId]) =>
        youtubeGet(
          'playlistItems',
          { part: 'contentDetails', playlistId, maxResults: String(Math.min(videosPerChannel, 50)) },
          auth,
        ),
      ),
    );
    ...
    results.forEach((res, index) => {
      ...
      for (const item of (parseJson(res.body)?.items as Raw[] | undefined) ?? []) {
        const videoId = str((item.contentDetails as Raw | undefined)?.videoId);
        if (!videoId) continue;
  ...
  for (const group of chunk(videoIds, 50)) {
    ...
    for (const item of (parseJson(res.body)?.items as Raw[] | undefined) ?? []) {
      ...
      const channelId = ownerByVideo.get(videoId) ?? str((item.snippet as Raw | undefined)?.channelId);
      const bucket = byChannel.get(channelId);
      if (!bucket) continue;
      const snippet = (item.snippet as Raw | undefined) ?? {};
      const stats = (item.statistics as Raw | undefined) ?? {};
      const tags = Array.isArray(snippet.tags) ? (snippet.tags as unknown[]).map((t) => str(t)).filter(Boolean) : [];
      bucket.uploads.push({
        videoId,
        title: str(snippet.title),
        channelId,
        ...
        durationSeconds: parseIsoDurationSeconds(str((item.contentDetails as Raw | undefined)?.duration)),
...
async function loadCommentGaps(
  ...
  const results = await Promise.all(
    targets.map((video) =>
      youtubeGet(
        'commentThreads',
        { part: 'snippet', videoId: video.videoId, order: 'relevance', maxResults: '8', textFormat: 'plainText' },
        auth,
      ),
    ),
  );
  ...
  results.forEach((res, index) => {
    ...
    const gaps: string[] = [];
    for (const item of (parseJson(res.body)?.items as Raw[] | undefined) ?? []) {
      const top = ((item.snippet as Raw | undefined)?.topLevelComment as Raw | undefined)?.snippet as Raw | undefined;
...
function collectQueries(input: TopicScoutInput): string[] {
...
export async function youtubeTopicScout(input: TopicScoutInput): Promise<ApiResult> {
  ...
  if (input.excludeChannelId) exclude.add(input.excludeChannelId);
  const mine = await ownChannelId(auth);
  if (mine) exclude.add(mine);
  ...
  for (const hit of found.hits) {
    if (exclude.has(hit.channelId) || seenChannel.has(hit.channelId)) continue;
  ...
  for (const channelId of uniqueIds) {
    ...
    const channelOutliers: OutlierVideo[] = [];
    for (const video of bucket.uploads) {
      if (video.views < minViews) continue;
      if (!isShortEnough(video.durationSeconds, duration)) continue;
