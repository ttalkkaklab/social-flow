...
import type { Alignment } from './sentence-spacing.js';
import { qwen3AsrTranscribeSchema, transcribeLocal, type Qwen3AsrSegment } from './qwen3-asr-client.js';

const isLetter = (c: string): boolean => /[\p{L}\p{N}]/u.test(c);

interface TimedLetter { char: string; start: number; end: number }

/** Every letter of the transcript with its word's span spread evenly across the word. */
export function transcriptLetters(words: Qwen3AsrSegment[]): TimedLetter[] {
  const out: TimedLetter[] = [];
 ...
 */
export function matchLetters(script: string[], heard: string[]): number[] {
  const n = script.length, m = heard.length;
  if (!n || !m) return script.map(() => -1);
  // Full DP table: a scene is a few hundred letters, so n×m stays small.
  const dp: Int32Array[] = [];
  for (let i = 0; i <= n; i++) { dp.push(new Int32Array(m + 1)); dp[i][0] = i; }
  ...
  while (i > 0 && j > 0) {
    ...
    if (dp[i][j] === dp[i - 1][j - 1] + (same ? 0 : 1)) { if (same) matched[i - 1] = j - 1; i--; j--; }
 ...
 */
export function alignmentFromWords(text: string, words: Qwen3AsrSegment[]): { alignment: Alignment; matched: number; letters: number } {
  const chars = [...text];
  ...
  const starts = new Array<number>(chars.length).fill(NaN), ends = new Array<number>(chars.length).fill(NaN);
  let count = 0;
  matched.forEach((h, k) => { if (h >= 0) { starts[letterIdx[k]] = heard[h].start; ends[letterIdx[k]] = heard[h].end; count++; } });
  if (!count) throw new Error('No script letter matched the transcript');
  ...
  for (let k = 0; k <= letterIdx.length; k++) {
    ...
    if (!known && k < letterIdx.length) continue;
    ...
    if (gapCount > 0) {
      ...
      for (let g = 1; g <= gapCount; g++) { const i = letterIdx[from + g]; starts[i] = t0 + (g - 1) * step; ends[i] = t0 + g * step; }
    }
    prev = k;
  }
  let cursor = 0;
  for (let i = 0; i < chars.length; i++) {
    if (Number.isNaN(starts[i])) { starts[i] = cursor; ends[i] = cursor; }
    else { starts[i] = Math.max(cursor, starts[i]); ends[i] = Math.max(starts[i], ends[i]); cursor = ends[i]; }
  }
 ...
 */
export async function alignmentFromAsr(wav: string, text: string, language: string): Promise<{ alignment: Alignment; matched: number; letters: number; transcript: string }> {
