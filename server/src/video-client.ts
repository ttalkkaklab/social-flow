 ...
 */
export const DEFAULT_VIDEO_MODEL: VideoModel = 'veo-3.1-fast-generate-preview';
...
const FILES_URI_PREFIX = `${API_BASE_URL}/files/`;
/** The server drops a generated video 48 hours after it was made, and with it the extension handle. */
const HANDLE_LIFETIME_MS = 48 * 60 * 60 * 1000;
...
function rejectLiteNegativePrompt(negativePrompt: string | undefined, model: VideoModel, ctx: z.RefinementCtx): void {
...
const negativePromptSchema = z
  ...
  .optional()
  .describe(
    'Comma-separated noun or adjective phrases to exclude (e.g. "wall, frame, text overlay"). ' +
...
// sourceVideoPath names the local copy (used to find its Files API handle and to
// report the source); sourceVideoUri overrides that lookup with a handle you already have.
export const videoExtensionSchema = z
  .object({
    ...
    sourceVideoUri: z
      .string()
      .startsWith(FILES_URI_PREFIX, `Source video URI must start with ${FILES_URI_PREFIX}`)
      .optional(),
    model: z.enum(VALID_VIDEO_MODELS).optional().default(DEFAULT_VIDEO_MODEL),
    outputPath: z.string().optional(),
    filename: bareFilenameSchema('video').optional(),
...
/**
 * The Files API handle of a saved video, written next to the mp4 as `<name>.mp4.veo.json`.
 *
 * Extension takes the video the server still holds, addressed by its Files API uri —
 * inline bytes are refused (`encodedVideo isn't supported by this model`, measured
 * 2026-09-06), and so is re-uploading the downloaded copy ("Input video must be a video
 * that was generated by VEO that has been processed"). The uri only exists in the
 * generation response, so it has to be kept at save time or it is gone for good.
 */
...
const HANDLE_SUFFIX = '.veo.json';

function writeVideoHandle(videoPath: string, handle: VideoHandle): void {
  try {
    fs.writeFileSync(`${videoPath}${HANDLE_SUFFIX}`, `${JSON.stringify(handle, null, 2)}\n`);
  } catch (error) {
    // The video itself is saved — a missing handle costs the extension path, not this call.
    console.error(`[Veo] Could not write the extension handle: ${error instanceof Error ? error.message : String(error)}`);
  }
}

function readVideoHandle(videoPath: string): VideoHandle | undefined {
  try {
    const parsed = JSON.parse(fs.readFileSync(`${videoPath}${HANDLE_SUFFIX}`, 'utf8')) as VideoHandle;
    return typeof parsed?.uri === 'string' && parsed.uri.startsWith(FILES_URI_PREFIX) ? parsed : undefined;
  } catch {
    return undefined;
  }
}

// ── Shared polling·saving helpers ────────────────────────────────

interface OperationStatus {
  done?: boolean;
  response?: {
    // new API shape (generateVideoResponse.generatedSamples)
    generateVideoResponse?: {
      generatedSamples?: Array<{ video?: { uri?: string } }>;
      // RAI (Responsible AI) content-filtering fields
...
async function awaitVideoAndSave(
  ...
  filename: string,
  label: string,
): Promise<{ videoPath: string; videoUri: string } | { error: string }> {
  let pollCount = 0;
  ...
  while (pollCount < MAX_POLLS) {
    ...
    // Keep the server-side handle beside the file — veo_extension can only take this uri.
    ...
    return { videoPath: fullPath, videoUri };
  }
 ...
 *
 * The input is addressed by its Files API uri, taken from the `<name>.mp4.veo.json`
 * handle written when the clip was generated. The API refuses the bytes of the local
 * copy, whether inline or re-uploaded, so a clip with no live handle cannot be extended
 * — see the VideoHandle comment for the two error messages.
 */
export async function extendVideo(request: VideoExtensionRequest): Promise<VideoGenerationResponse> {
  ...
  try {
    ...
    const sourceVideoUri = request.sourceVideoUri || handle?.uri;
    if (!sourceVideoUri) {
      return {
        success: false,
        error:
          `No Veo file handle for ${request.sourceVideoPath} (expected ${request.sourceVideoPath}${HANDLE_SUFFIX}). ` +
          'Extension takes the copy the Gemini server still holds, addressed by its Files API uri; ' +
          'it refuses the bytes of a local file. Generate the clip again to get a handle, or pass sourceVideoUri.',
      };
    }
    ...
    if (!request.sourceVideoUri && handle) {
      ...
      if (!Number.isFinite(savedAtMs)) {
        return {
          success: false,
          error:
            `The Veo file handle for ${request.sourceVideoPath} carries no readable savedAt (${JSON.stringify(handle.savedAt)}), ` +
      ...
      if (Date.now() - savedAtMs > HANDLE_LIFETIME_MS) {
        return {
          success: false,
          error:
            `The Veo file handle for ${request.sourceVideoPath} was made at ${handle.savedAt} and the server keeps a generated video for 48 hours only. ` +
