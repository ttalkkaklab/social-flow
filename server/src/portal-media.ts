...
class MediaTooLarge extends Error {
  constructor(readonly bytes: number) { super('media exceeds upload limit'); }
}
function recordOversize(dir: string, file: string, kind: MediaKind, bytes: number) {
  const result = { skipped: true as const, reason: 'oversized' as const, kind, file, byteSize: bytes, limitBytes: limits[kind] * 1024 * 1024 };
...
function readMedia(file: string, kind: MediaKind) {
  ...
  try {
    ...
    if (!stat.isFile() || !stat.size) throw new Error(`${kind} must be a regular nonempty file.`);
    if (stat.size > limit) throw new MediaTooLarge(stat.size);
    ...
    if (size > limit) throw new MediaTooLarge(size);
...
export async function uploadShotMedia(args: z.infer<typeof mediaUploadSchema>, fetchImpl?: FetchLike) {
  ...
  if (!file.startsWith(root + path.sep)) throw new Error('Media must be inside the episode directory, including symlinks.');
  ...
  const payload = buildImportPayload(dir), shots = payload.scenes as Record<string, unknown>[];
  ...
  let fd: number;
  try { fd = openSync(lock, 'wx'); } catch { throw new Error('Another media upload is active for this episode. After a crash, inspect .portal-media.lock before removing it.'); }
  let uploaded: unknown, revisionNo: number | undefined, recoveryFile: string | undefined;
  let phase = 'validate';
  try {
    const { data: head } = await client.getEpisode(state.episodeId);
    if (head.headRevisionNo !== state.headRevisionNo) throw new Error('Portal head moved. Side-pull and merge before uploading; no blob or checkpoint sent.');
    ...
    recoveryFile = path.join(backup, 'uploaded-scenes.js'); writeFileSync(recoveryFile, next);
    ...
    try { writeFileSync(tmp, next); renameSync(tmp, scenesFile); } finally { rmSync(tmp, { force: true }); }
    writePortalState(dir, { headRevisionNo: revisionNo });
    return { skipped: false as const, episodeId: state.episodeId, revisionNo, kind: args.kind, id: data.id, sha256: data.sha256 };
  } catch (error) {
    if (error instanceof MediaTooLarge) return recordOversize(dir, file, args.kind, error.bytes);
...
export type MediaOutcome = { portalMedia?: { error?: string; [key: string]: unknown } };
/** This function wraps real generation handlers; awaiting previz upload precedes the vendor callback. */
export async function withShotMedia<T extends object>(args: unknown, kind: MediaKind, run: () => Promise<T>, output: (result: T) => string | undefined, fetchImpl?: FetchLike): Promise<T & MediaOutcome> {
  ...
  if (kind === 'video') {
    if (!target.previzFile) throw new Error('Pass portal.previzFile. Video generation has not been called.');
    await uploadShotMedia({ ...target, kind: 'previz', file: target.previzFile }, fetchImpl);
  }
  const result = await run(), file = output(result);
  if (!file) return result;
  try { return { ...result, portalMedia: await uploadShotMedia({ ...target, kind, file }, fetchImpl) }; }
  catch (error) { return { ...result, portalMedia: { error: describePortalError(error), file, next: 'Media already exists. Retry portal_shot_media_upload; do not repeat generation.' } }; }
}
export function portalMediaReport(result: MediaOutcome) { return result.portalMedia ? `\n\nPortal media: ${JSON.stringify(result.portalMedia)}` : ''; }

export function withoutPortal(args: unknown): unknown {
  if (!args || typeof args !== 'object' || Array.isArray(args)) return args;
  const { portal: _portal, ...rest } = args as Record<string, unknown>;
  return rest;
}
