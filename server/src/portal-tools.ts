...
import { describePortalError, portalClientFor, SAFE_DOCUMENT_NAME, type FetchLike, type PortalClient } from './portal-client.js';
import {
  buildImportPayload,
  channelOfEpisodeDir,
  DOCUMENT_FILES,
...
export const workspaceCheckSchema = z.object({ channel: channelArg });
  ...
  return {
    async workspaceCheck({ channel }) {
      ...
      try {
        ...
        return ok({ channel: r.channel ?? null, workspace: r.client.workspace, source: r.client.source, holder: r.client.holder, ...(data as object) });
      } catch (error) {
        return failed(error);
      }
    },
