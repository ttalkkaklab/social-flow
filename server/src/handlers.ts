...
export const ROUTES: Record<string, (args: unknown) => Promise<ToolResult>> = {
  ...PORTAL_API_ROUTES as Record<string, (args: unknown) => Promise<ToolResult>>,
  ...UNIT_ROUTES,
  ...REVIEW_ROUTES,
  ...
  portal_shot_media_upload: async (args) => {
    try { return text(JSON.stringify(await uploadShotMedia(parseArgs(mediaUploadSchema, args)))); }
    catch (error) { return text(error instanceof Error ? error.message : String(error), true); }
  },
