...
Returns: JSON — { id, slug, title, stage, url, pageUrl }.`,
  ...
  },
  {
    ...
    annotations: { readOnlyHint: false, destructiveHint: false, idempotentHint: true, openWorldHint: true },
    description: `Checkpoint the episode on the portal when a stage ends — candidates after the three scenario pages, scenario after the pick, board after scenes.js is written. With episodeDir the shots and meta from storyboard/scenes.js (when present) and the standard documents that exist (research.md · storyboard.md · script.md · storyboard.html) go up together; identical content makes no new revision and only moves the stage. baseRevisionNo defaults to .portal.json's head; when it differs from the portal's head the answer is 409 head_moved with what moved since your base (shots · meta · documents) — another machine saved first: portal_storyboard_pull, re-apply only what it names, checkpoint again. Never retry a 409 blind.
