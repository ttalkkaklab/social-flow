...
export const TOOLS: Tool[] = [
  ...
  },
  {
    ...
    description:
      '공공데이터포털 파일데이터 원본 다운로드 (무인증) — CSV 등 실파일을 로컬에 저장하고 인코딩 판별 + 선두 6행 미리보기를 반환한다(100MB 상한). publicDataPk/publicDataDetailPk 는 datago_detail 응답값을 그대로 쓴다. 활용신청 없이 즉시 쓸 수 있는 가장 빠른 파일 수집 경로다. 미리보기 encoding 이 euc-kr 이면 Read 전에 iconv 변환이 필요하다. saveDir 는 주제 디렉토리(data/<카테고리>/<주제>/storyboard/) 하위를 권장 — 생략 시 임시 디렉토리에 저장된다.',
    inputSchema: {
      type: 'object',
      properties: {
...
Returns: a text block with the saved .mp4 file path, reference image list, model, aspect ratio, resolution, and duration.`,
  ...
  // 자격증명 파일이 있는 채널의 툴만 ListTools 에 노출된다 (index.ts + SNS_CHANNEL_BY_TOOL).
  {
    ...
    description:
      `⚠️ Threads 직접 게시 — 로컬 토큰으로 Threads API 에 **즉시 공개 게시**한다(게시 계정은 토큰의 /me 로 자동 결정). 별도 검토 게이트가 없으므로 반드시 사용자가 최종 문안·미디어를 확인·승인한 직후에만 호출한다(HITL — 승인 없이 호출 금지). 텍스트 또는 이미지 1장 — 영상 불가(영상 콘텐츠는 커버 이미지 본문 + 풀영상 링크 답글 방식). 본문 링크 금지 채널 규칙: 링크는 본문이 아니라 게시 성공 직후 응답 postId 를 replyToId 로 넣은 답글로 이어서 게시한다. ${SNS_HITL_LINE}`,
    inputSchema: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    description:
      `⚠️ Instagram 직접 게시 — 로컬 토큰으로 IG API 에 **즉시 공개 게시**한다(게시 계정은 토큰의 /me 로 자동 결정). 별도 검토 게이트가 없으므로 반드시 사용자가 최종 문안·미디어를 확인·승인한 직후에만 호출한다(HITL — 승인 없이 호출 금지). 이미지 1~10장(2장 이상 캐러셀, JPEG 권장) **또는** 릴스 영상 1개(imageUrls 와 배타). 게시 후 이미지 교체 불가(캡션만 수정 가능). ${SNS_HITL_LINE}`,
    inputSchema: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    description:
      `⚠️ YouTube 직접 게시 — 로컬 OAuth 리프레시 토큰으로 **로컬 영상 파일을 즉시 공개 업로드**한다(대상 채널은 토큰 소유 계정으로 자동 결정). 별도 검토 게이트가 없으므로 반드시 사용자가 최종 문안·영상을 확인·승인한 직후에만 호출한다(HITL — 승인 없이 호출 금지). 세로 9:16·3분 이하 영상은 쇼츠로 자동 분류(별도 플래그 없음). 유일하게 공개 URL 호스팅이 불필요한 채널이다(파일 직접 업로드). 업로드 쿼터 일 6건. ${SNS_HITL_LINE}`,
    inputSchema: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    description:
      'SNS 게시용 로컬 자격증명을 일괄 점검한다 — Threads/Instagram/Facebook 페이지는 /me 조회, YouTube 는 리프레시 토큰 교환 + 채널 조회. 계정 id·이름과 유효 여부만 반환하며 토큰 값은 노출하지 않는다. 채널별 *_publish 전 사전 점검과 Meta 토큰 60일 만료 갱신 시점 판단에 사용한다. 자격증명 미설정 채널은 사유와 함께 ok:false 로 표시된다.',
 ...
 */
export const SNS_CHANNEL_BY_TOOL: Record<string, 'THREADS' | 'INSTAGRAM' | 'FACEBOOK' | 'YOUTUBE'> = {
