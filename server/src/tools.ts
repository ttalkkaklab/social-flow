...
const CONTENT_FEEDBACK_OUTPUT: OutputSchema = {
  ...
  properties: {
    ...
    youtube: {
      type: 'object',
      description:
        '{ available, error?, account, cohort, items[], notes[] } — items carries, per recent video, hook (% getting past the opening), retain (average % watched), shareRate (shares against views, since YouTube reports no reach), angle (views low while hook/retention held up), and problem/hypothesis/next-episode notes',
    },
    instagram: {
      type: 'object',
      description:
