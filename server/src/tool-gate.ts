...
export interface ToolGateDecision {
  enabled: boolean;
  /** One line for logs and CallTool refusals. */
  reason: string;
}

export interface ResolveToolGateOptions {
  jsonPatterns?: string[];
  jsonFile?: string;
  env?: EnvMap;
}
...
function read(env: EnvMap, key: string): string | undefined {
  ...
  if (upper !== key && Object.prototype.hasOwnProperty.call(env, upper)) return env[upper];
  return undefined;
}
...
export function toolOverrideVar(name: string): string {
...
export function matchesJsonPattern(name: string, pattern: string): boolean {
  return pattern.endsWith('*') ? name.startsWith(pattern.slice(0, -1)) : name === pattern;
}

function listMatches(name: string, patterns: string[]): boolean {
  return patterns.some((pattern) => matchesPattern(name, pattern));
}

function parseFlag(raw: string, source: string): boolean {
...
export function parseToolFlags(raw: string | undefined): Map<string, string> {
...
function flagFor(name: string, flags: Map<string, string>): { pattern: string; raw: string } | undefined {
  // Exact match beats a prefix — `veo_*=off` and `veo_img2video=on` can coexist.
  let prefixHit: { pattern: string; raw: string } | undefined;
 ...
 */
export function isToolEnabled(name: string, env: EnvMap = process.env): ToolGateDecision {
 ...
 */
export function resolveToolGate(name: string, opts: ResolveToolGateOptions = {}): ToolGateDecision {
...
export function warnUnknownPatterns(knownNames: readonly string[], env: EnvMap = process.env): string[] {
  const known = new Set(knownNames.map((n) => n.toLowerCase()));
  const warnings: string[] = [];
  const check = (varName: string) => {
    for (const pattern of parsePatternList(read(env, varName))) {
      ...
      if (!known.has(lower)) warnings.push(`${varName}: unknown tool "${pattern}"`);
    }
  };
  check(ALLOW_VAR);
  check(DENY_VAR);
