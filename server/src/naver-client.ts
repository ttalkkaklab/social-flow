...
export type NaverSearchType = 'news' | 'blog' | 'web' | 'cafe';

const ENDPOINT_BY_TYPE: Record<NaverSearchType, string> = {
  news: 'news.json',
  blog: 'blog.json',
  web: 'webkr.json',
...
export interface NaverSearchInput {
  ...
  /** 결과 수 (기본 10, 최대 30 — API 상한은 100이지만 LLM 컨텍스트 보호) */
  display?: number;
  /** 시작 위치 (1부터) — 첫 페이지에 근거가 없을 때만 */
  start?: number;
  /** sim=정확도순(기본), date=최신순 — web 타입은 정렬 미지원 */
  sort?: 'sim' | 'date';
}
