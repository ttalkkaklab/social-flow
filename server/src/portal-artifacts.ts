...
import { portalClientFor, type FetchLike } from './portal-client.js';
...
function safeRead(file: string, max = 10_000): string | undefined {
  if (!existsSync(file)) return undefined;
  const value = readFileSync(file, 'utf8');
  if (Buffer.byteLength(value) > max) throw new Error(`${path.relative(process.cwd(), file)} exceeds ${max} bytes.`);
  return value.trim();
}
...
function youtubeMeta(source: string) {
  ...
  if (!title || !description) throw new Error('output/youtube/meta.md needs ## title and ## description.');
  const parsedTags = hashtags(tags);
  return { platform: 'youtube' as const, title, description, hashtags: parsedTags.length ? parsedTags : tags.split(/[,\n]/).map(v => v.trim()).filter(Boolean).map(v => v.startsWith('#') ? v : `#${v}`) };
}
...
function readOutputs(dir: string) {
  ...
  const outputs: z.infer<typeof output>[] = [];
...
function readCosts(dir: string) {
  const tally = path.join(dir, '.work', 'cost-tally.tsv');
  if (!existsSync(tally)) return undefined;
  ...
  for (const row of readFileSync(tally, 'utf8').split(/\r?\n/)) {
    ...
    if (!item || !Number.isFinite(quantity) || quantity < 0) throw new Error(`Invalid cost tally line: ${row}`);
    const unit = prices.get(item);
    const costUsd = typeof unit === 'number' && Number.isFinite(unit) ? unit * quantity : null;
    if (costUsd === null) unpricedItems.add(item); else actualUsd += costUsd;
...
function readSmallMedia(file: string) {
  const fd = openSync(file, 'r');
  try {
    const stat = fstatSync(fd);
    if (!stat.isFile() || !stat.size) throw new Error('Cover must be a regular nonempty file.');
    if (stat.size > 5 * 1024 * 1024) throw new Error('Cover exceeds the 5 MiB image limit.');
    const bytes = Buffer.allocUnsafe(stat.size); let size = 0;
    while (size < bytes.length) { const n = readSync(fd, bytes, size, bytes.length - size, null); if (!n) break; size += n; }
    return bytes.subarray(0, size);
  } finally { closeSync(fd); }
}

function resolveEpisodeId(dir: string | undefined, explicit: string | undefined): string {
  const recorded = dir ? readPortalState(dir)?.episodeId : undefined;
  if (recorded && explicit && recorded !== explicit) throw new Error('episodeId conflicts with .portal.json.');
  const id = explicit ?? recorded;
  if (!id) throw new Error('episodeId or an episodeDir with .portal.json is required.');
  return id;
}

export async function syncEpisodeArtifacts(args: z.infer<typeof artifactSyncSchema>, fetchImpl?: FetchLike) {
  ...
  if (state?.workspace && state.workspace !== client.workspace) throw new Error(`Workspace mismatch — .portal.json names "${state.workspace}" but the key opens "${client.workspace}". Nothing was sent.`);
  ...
  if (args.uploadCover && existsSync(cover)) {
    ...
    if (!file.startsWith(root + path.sep)) throw new Error('Cover must stay inside this episode.');
  ...
  if (!Object.keys(body).length) throw new Error('No platform copy or cost tally found.');
  return (await client.updateArtifacts(episodeId, body)).data;
}

export async function recordPublication(args: z.infer<typeof publicationRecordSchema>, fetchImpl?: FetchLike) {
