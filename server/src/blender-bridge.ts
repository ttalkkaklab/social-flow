...
async function runBridge<T>(job: Record<string, unknown>, timeoutMs: number): Promise<BridgeResult<{ result: T }>> {
...
export async function renderPreviz(request: BlenderRenderPrevizRequest): Promise<BridgeResult<BlenderPrevizResult>> {
  ...
  const framesForTimeout =
    request.frameStart !== undefined && request.frameEnd !== undefined ? request.frameEnd - request.frameStart + 1 : DEFAULT_FRAME_END;
