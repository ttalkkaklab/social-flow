 ...
 * to do anyway.
 */
export const SUPERTONIC_VOICE_NAMES = ['F1', 'F2', 'F3', 'F4', 'F5', 'M1', 'M2', 'M3', 'M4', 'M5'] as const;
export type SupertonicVoice = (typeof SUPERTONIC_VOICE_NAMES)[number];

/** Matches the upstream CLI default — the same arguments producing different results would surprise. */
export const DEFAULT_SUPERTONIC_VOICE: SupertonicVoice = 'M1';
...
export type SupertonicGenerateRequest = z.infer<typeof supertonicGenerateSchema>;
...
export interface SupertonicResponse {
  ...
  voice?: string;
  lang?: string;
  /** Synthesized audio duration (seconds) — Python returns it directly so scene-length checks can use it. */
  durationSeconds?: number;
  sampleRate?: number;
  /** Sentence groups the model read one at a time, with `chunkPause` of silence between them. */
  groups?: SupertonicGroup[];
...
/** One sentence group as synthesized: the text the model read in one pass and its span in the WAV. */
export interface SupertonicGroup { text: string; start: number; end: number }
...
export async function generateLocalSpeech(request: SupertonicGenerateRequest): Promise<SupertonicResponse> {
  ...
  const outFile = resolveOutputFile(
    ...
    request.filename || `supertonic_${Date.now()}.wav`,
    'audio',
  );
  ...
  console.error(
    `[Supertonic] Synthesizing locally... (voice: ${request.voice}, lang: ${request.lang}, ${request.text.length} chars)`,
  );
  ...
  try {
    stdout = await new Promise<string>((resolve, reject) => {
      execFile(
        ...
        { timeout: timeoutFor(request.text.length), maxBuffer: 1024 * 1024 },
        (error, out, errOut) => {
          if (error) {
