...
export function readIdentity(dir: string): { key: string; name?: string; role?: string; appearance?: string } {
  ...
  if (!existsSync(file)) throw new Error(`${file} not found. Nothing was sent.`);
...
async function byKey(client: PortalClient, key: string): Promise<PortalCharacterRecord | null> {
  const { data } = await client.listCharacters({ key });
  return data.items[0] ?? null;
}

export async function getCharacter(client: PortalClient, args: z.infer<typeof characterGetSchema>) {
  const record = args.id ? (await client.getCharacter(args.id)).data : await byKey(client, args.key!);
  if (!record) throw new Error(`No character with key "${args.key}" in workspace ${client.workspace}.`);
  return summarize(record);
}

export async function createCharacter(client: PortalClient, args: z.infer<typeof characterCreateSchema>, channel: string | undefined) {
...
export async function updateCharacter(client: PortalClient, args: z.infer<typeof characterUpdateSchema>) {
  const { id: _id, channel: _c, episodeDir: _d, ...patch } = args;
  const { data } = await client.updateCharacter(args.id, patch);
  return summarize(data);
}

export async function deleteCharacter(client: PortalClient, args: z.infer<typeof characterDeleteSchema>) {
...
export async function setCharacterTts(client: PortalClient, args: z.infer<typeof characterTtsSetSchema>) {
