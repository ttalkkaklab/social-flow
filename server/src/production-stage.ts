...
export interface StageEntry {
  stage: Stage;
  /** Buzz event ID of the human message that opened this stage. The proof a person asked for it. */
  humanEventId: string;
  at: string;
  note?: string;
}
...
export interface ProductionState {
  ...
  stage: Stage;
  history: StageEntry[];
  /** filename → sha256, snapshotted when the episode leaves the still gate. */
  approvedStills?: Record<string, string>;
}

/** Tools that may only run while the episode stands at one of these stages. */
const TOOL_STAGES: Array<{ match: RegExp; allow: Stage[]; label: string }> = [
  {
...
// TTS, music, search, insight and scout tools carry no stage requirement on purpose — they get
// re-run at many points for measurement, and blocking them would stop the work without
// protecting anything.
 ...
 */
export function findEpisodeDir(path: string): string | undefined {
  if (!path.includes(GATED_PATH_MARK)) return undefined;
  ...
  while (dir.includes(GATED_PATH_MARK)) {
    if (existsSync(join(dir, STATE_FILENAME))) return dir;
...
export function readState(episodeDir: string): ProductionState | undefined {
  const file = join(episodeDir, STATE_FILENAME);
  if (!existsSync(file)) return undefined;
  try {
    return JSON.parse(readFileSync(file, 'utf8')) as ProductionState;
  } catch {
    return undefined;
  }
}

export function writeState(episodeDir: string, state: ProductionState): void {
...
export function hashStills(episodeDir: string): Record<string, string> {
...
export function changedStills(episodeDir: string, state: ProductionState): string[] {
...
function stageRule(toolName: string) {
 ...
 */
export function checkStageGate(toolName: string, args: unknown): GateVerdict {
  ...
  for (const episodeDir of episodeDirs) {
    ...
    if (!rule.allow.includes(state.stage)) {
      return {
        allowed: false,
        reason:
          `${rule.label}은 ${rule.allow.join(' 또는 ')} 단계에서만 됩니다. ` +
    ...
    if (drifted.length > 0) {
      return {
        allowed: false,
        reason:
          `${state.episode}의 스틸이 승인된 것과 다릅니다: ${drifted.join(', ')}.\n` +
