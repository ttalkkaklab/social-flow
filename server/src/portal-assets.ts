...
import { z } from 'zod';
import type { PortalAsset, PortalClient } from './portal-client.js';
...
export const ASSET_TYPES = ['video', 'music', 'sfx', 'image'] as const;
export const ASSET_ASPECTS = ['16:9', '9:16', '1:1', '4:3', '3:4', 'other'] as const;
export const ASSET_SORTS = ['relevance', 'newest', 'duration', '-duration'] as const;
...
export type AssetsSearchInput = z.infer<typeof assetsSearchSchema>;
...
export type AssetsGetInput = z.infer<typeof assetsGetSchema>;
...
const EXT: Record<string, string> = { 'video/mp4': 'mp4', 'audio/mpeg': 'mp3', 'audio/wav': 'wav', 'audio/mp4': 'm4a', 'image/png': 'png', 'image/jpeg': 'jpg', 'image/webp': 'webp' };

/** The search answer keeps the fields a skill decides on; prompt and extra stay on `get`. */
export function summarizeAsset(a: PortalAsset) {
...
export async function searchAssets(client: PortalClient, args: AssetsSearchInput) {
...
function fileStem(a: PortalAsset): string {
...
export function assetTarget(a: PortalAsset, shot?: number): { relative: string; clip?: string } {
...
export function assetLicense(client: PortalClient, a: PortalAsset) {
...
export async function getAsset(client: PortalClient, args: AssetsGetInput) {
  ...
  if (!existsSync(dir) || !statSync(dir).isDirectory()) throw new Error(`episodeDir is not a directory: ${dir}`);
  ...
  if (existsSync(file)) {
    if (!statSync(file).isFile()) throw new Error(`Target exists and is not a file: ${file}`);
    if (statSync(file).size === asset.byteSize && sha256File(file) === asset.sha256) return { ...result, downloaded: false, note: 'Same sha256 already on disk — not downloaded again.' };
  ...
  try {
    ...
    closeSync(fd);
    if (size !== asset.byteSize || hash.digest('hex') !== asset.sha256) throw new Error(`Downloaded bytes do not match the portal's sha256/byteSize for asset ${asset.id}. Nothing was kept.`);
    renameSync(tmp, file);
  } catch (error) {
