...
import { SNS_CHANNELS, snsCredentialFiles, type SnsChannel } from './config.js';
...
export function enabledChannels(): SnsChannel[] {
...
async function loadTokenFile(channel: SnsChannel): Promise<{ token?: string; error?: ApiResult }> {
  ...
  try {
    ...
    return {
      error: fail(
        400,
        `Token file not found: ${filePath} — ${channel} 게시에는 로컬 토큰이 필요하다 (SNS_TOKEN_DIR 또는 채널별 *_TOKEN_FILE env 로 위치 변경 가능).`,
      ),
    };
  }
}
...
export interface FacebookPublishInput {
  ...
  imageUrls?: string[];
  videoUrl?: string;
  /** 링크 첨부(텍스트 게시에서만) — FB 채널 문법의 "링크 첨부 또는 첫 댓글" 규칙용 */
  linkUrl?: string;
}
...
async function loadYoutubeClient(): Promise<{ client?: YoutubeOauthClient; error?: ApiResult }> {
...
export const COMMENT_CHANNELS = ['THREADS', 'INSTAGRAM', 'FACEBOOK'] as const;
export type CommentChannel = (typeof COMMENT_CHANNELS)[number];

/** 정규화 댓글 — 채널별 필드명 차이를 흡수해 분류·우선순위 판단만 남긴다. */
...
async function inboxThreads(input: Required<Pick<CommentInboxInput, 'postLimit' | 'commentLimit'>>, now: number): Promise<ChannelInbox> {
  const { token, error } = await loadTokenFile('THREADS');
  if (!token) return { error };
  const me = await fetchMe(THREADS_BASE, token, 'id,username');
...
async function inboxInstagram(input: Required<Pick<CommentInboxInput, 'postLimit' | 'commentLimit'>>, now: number): Promise<ChannelInbox> {
  const { token, error } = await loadTokenFile('INSTAGRAM');
  if (!token) return { error };
  const me = await fetchMe(IG_BASE, token, 'id,username');
  if (!me.ok) return { error: me };
...
async function inboxFacebook(input: Required<Pick<CommentInboxInput, 'postLimit' | 'commentLimit'>>, now: number): Promise<ChannelInbox> {
  const { token, error } = await loadTokenFile('FACEBOOK');
  if (!token) return { error };
  const me = await fetchMe(FB_BASE, token, 'id,name');
  if (!me.ok) return { error: me };
...
export async function commentInbox(input: CommentInboxInput = {}): Promise<ApiResult> {
  ...
  const available = new Set<string>(enabledChannels());
  const requested = input.channels?.length ? input.channels : [...COMMENT_CHANNELS];

  const collectors: Record<CommentChannel, (l: typeof limits, n: number) => Promise<ChannelInbox>> = {
  ...
  for (const channel of requested) {
    ...
    if (result.account) accounts[channel] = result.account;
    if (result.error) {
  ...
  for (const comment of actionable) byChannel[comment.channel] = (byChannel[comment.channel] ?? 0) + 1;
...
export async function checkAccounts(): Promise<ApiResult> {
