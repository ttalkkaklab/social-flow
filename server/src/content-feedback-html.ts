...
import type { FeedbackReport, FeedbackStep, PlatformSection, ReviewedItem, Tone } from './content-feedback.js';
...
function shortDate(iso: string | null): string {
  ...
  if (Number.isNaN(d.getTime())) return escapeHtml(iso.slice(0, 10));
  return `${d.getMonth() + 1}.${d.getDate()}`;
}

function clip(text: string, n = 36): string {
  const t = text.replace(/\s+/g, ' ').trim();
  return t.length <= n ? t : `${t.slice(0, n - 1)}…`;
}

function toneLabel(tone: Tone): string {
  if (tone === 'ok') return '유지';
  if (tone === 'watch') return '손볼 점';
  if (tone === 'fix') return '손대기';
  return '대기';
}

function countTones(items: ReviewedItem[]): Record<Tone, number> {
  return {
...
function barRow(label: string, value: number | null, max: number, kind: 'good' | 'bad'): string {
...
function chartBlock(title: string, items: ReviewedItem[], key: string, higherIsBetter: boolean): string {
  const values = items.map((i) => i.metrics[key]).filter((n): n is number => n != null);
  const max = Math.max(1, ...values, ...(items.map((i) => i.metrics[key]).filter((n): n is number => n != null)));
  const rows = items
...
function funnel(steps: Array<{ k: string; v: string; hint: string }>, accent: string): string {
  const cells = steps
    .map(
      (s, i) => `<li class="fn-step ${accent}">
...
function rail(step: FeedbackStep): string {
...
function itemCard(item: ReviewedItem, metricCells: string): string {
...
function metricCell(k: string, v: string, vs?: string): string {
  return `<div class="mcell ${vs ?? ''}"><span>${escapeHtml(k)}</span><strong>${escapeHtml(v)}</strong></div>`;
}

function actionBoard(section: PlatformSection): string {
  const seen = new Set<string>();
  const actions: string[] = [];
  for (const item of section.items) {
    for (const step of item.steps) {
      if (step.lever === 'pending') continue;
      if (seen.has(step.next)) continue;
      seen.add(step.next);
      const leverLabel =
        step.lever === 'hook' ? '훅' : step.lever === 'retain' ? '유지' : step.lever === 'angle' ? '각도' : '공유';
...
function platformSection(section: PlatformSection, title: string, accent: string): string {
  ...
  const cards = section.items
    .map((item) => {
      if (section.platform === 'YOUTUBE') {
        return itemCard(
          item,
          metricCell('조회', fmt(item.metrics.views), vsClass(item.vsCohort.views)) +
      ...
      return itemCard(
        item,
        metricCell('도달', fmt(item.metrics.reach), '') +
...
function vsClass(vs: string | undefined): string {
  if (vs === 'above') return 'up';
  if (vs === 'below') return 'down';
  return '';
}

export function renderFeedbackHtml(report: FeedbackReport): string {
