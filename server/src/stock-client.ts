...
export const STOCK_PROVIDERS = ['pexels', 'pixabay', 'nasa', 'commons'] as const;
export const STOCK_MEDIA = ['video', 'photo'] as const;
export const STOCK_ORIENTATIONS = ['portrait', 'landscape', 'square', 'any'] as const;
...
export type StockMedia = (typeof STOCK_MEDIA)[number];
export type StockOrientation = (typeof STOCK_ORIENTATIONS)[number];

/** The block the storyboard stores as `visual.license` — render-routing.js checkLicense reads it verbatim */
...
export interface StockItem {
  ...
  width?: number;
  height?: number;
  /** seconds — video only, when the provider reports it */
  duration?: number;
  thumbnail?: string;
  /** largest first — the storyboard downloads files[0] unless a smaller one is enough */
  files: StockFile[];
...
const stripTags = (v: unknown): string | undefined => str(String(v ?? '').replace(/<[^>]*>/g, '').replace(/\s+/g, ' '));
...
function orientationOf(width?: number, height?: number): StockOrientation | undefined {
  if (!width || !height) return undefined;
  if (Math.abs(width - height) / Math.max(width, height) < 0.05) return 'square';
  return height > width ? 'portrait' : 'landscape';
}

function keep(item: StockItem, input: StockSearchInput): boolean {
  ...
  if (item.media === 'video' && item.duration !== undefined) {
    if (input.minDuration !== undefined && item.duration < input.minDuration) return false;
    if (input.maxDuration !== undefined && item.duration > input.maxDuration) return false;
  }
  return true;
}
...
async function pexels(input: StockSearchInput, media: StockMedia, limit: number): Promise<ProviderOutcome> {
  ...
  const json = (await getJson(base + buildQuery({ query: input.query, orientation, per_page: Math.min(80, limit * 2), locale: input.locale }),
    { Authorization: key })) as Record<string, any>;
  const rows = ((media === 'video' ? json.videos : json.photos) as any[]) ?? [];
  const items = rows.map((r): StockItem => {
    const author = str(r.user?.name) ?? str(r.photographer);
    const authorUrl = str(r.user?.url) ?? str(r.photographer_url);
    const files: StockFile[] = media === 'video'
      ? ((r.video_files as any[]) ?? [])
          .filter((f) => !f.file_type || /mp4/i.test(String(f.file_type)))
          .map((f) => compact({ url: String(f.link), width: num(f.width), height: num(f.height), label: str(f.quality), mime: str(f.file_type) }) as StockFile)
...
async function pixabay(input: StockSearchInput, media: StockMedia, limit: number): Promise<ProviderOutcome> {
  ...
  const json = (await getJson(base + buildQuery(params))) as Record<string, any>;
  const items = ((json.hits as any[]) ?? []).map((r): StockItem => {
    const author = str(r.user);
    let files: StockFile[], width: number | undefined, height: number | undefined, thumbnail: string | undefined;
    if (media === 'video') {
      const v = (r.videos ?? {}) as Record<string, any>;
      files = ['large', 'medium', 'small', 'tiny']
        .filter((k) => str(v[k]?.url))
        .map((k) => compact({ url: String(v[k].url), width: num(v[k].width), height: num(v[k].height), label: k, mime: 'video/mp4' }) as StockFile);
      ...
      width = num(r.imageWidth); height = num(r.imageHeight);
...
async function nasa(input: StockSearchInput, media: StockMedia, limit: number): Promise<ProviderOutcome> {
  const page = Math.min(NASA_MAX, limit);
  const search = (await getJson('https://images-api.nasa.gov/search' +
    buildQuery({ q: input.query, media_type: media === 'video' ? 'video' : 'image', page_size: page }))) as Record<string, any>;
  const rows = ((search.collection?.items as any[]) ?? []).slice(0, page);
  const manifests = await Promise.all(rows.map(async (r) => {
    const id = str(r.data?.[0]?.nasa_id);
    if (!id) return [] as string[];
    try {
      const m = (await getJson(`https://images-api.nasa.gov/asset/${encodeURIComponent(id)}`)) as Record<string, any>;
      return ((m.collection?.items as any[]) ?? []).map((x) => String(x.href));
    } catch {
      return [] as string[];
    }
  }));
  const items = rows.map((r, i): StockItem | null => {
    const d = (r.data?.[0] ?? {}) as Record<string, any>;
    const id = str(d.nasa_id);
    if (!id) return null;
    const hrefs = manifests[i].map((h) => h.replace(/^http:\/\//, 'https://'));
    const rank = (h: string) => (/~orig\./.test(h) ? 0 : /~large\./.test(h) ? 1 : /~medium\./.test(h) ? 2 : /~small\./.test(h) ? 3 : 9);
    const wanted = media === 'video' ? /\.mp4$/i : /\.(jpe?g|png|tiff?)$/i;
    const files = hrefs
      .filter((h) => wanted.test(h) && !/~thumb|~preview|~mobile|_\d\.jpg$/i.test(h))
      .sort((a, b) => rank(a) - rank(b))
      .map((h): StockFile => ({ url: h, label: (h.match(/~([a-z]+)\.[a-z0-9]+$/i) ?? [])[1] ?? 'file', mime: media === 'video' ? 'video/mp4' : 'image/jpeg' }));
    const center = str(d.center);
    const thumb = ((r.links as any[]) ?? []).map((l) => str(l.href)).find(Boolean);
    return compact({
...
export function commonsLicenseAllowed(code: string | undefined, shortName: string | undefined): boolean {
...
async function commons(input: StockSearchInput, media: StockMedia, limit: number): Promise<ProviderOutcome> {
  ...
  const json = (await getJson(url, { 'User-Agent': USER_AGENT })) as Record<string, any>;
  let dropped = 0;
  const items: StockItem[] = [];
  for (const p of ((json.query?.pages as any[]) ?? [])) {
    const ii = (p.imageinfo?.[0] ?? {}) as Record<string, any>;
    const meta = (ii.extmetadata ?? {}) as Record<string, { value?: unknown }>;
    const m = (k: string) => stripTags(meta[k]?.value);
    ...
    if (!commonsLicenseAllowed(m('License'), m('LicenseShortName'))) { dropped++; continue; }
    ...
    const files: StockFile[] = [{ url: String(ii.url), width: num(ii.width), height: num(ii.height), label: 'original', mime }];
    if (media === 'photo' && str(ii.thumburl)) files.push({ url: String(ii.thumburl), width: num(ii.thumbwidth), height: num(ii.thumbheight), label: 'scaled (1280w)', mime });
    items.push(compact({
...
const RUNNERS: Record<StockProvider, (input: StockSearchInput, media: StockMedia, limit: number) => Promise<ProviderOutcome>> = {
  pexels, pixabay, nasa, commons,
};

export async function stockSearch(input: StockSearchInput): Promise<StockSearchResult> {
  const media: StockMedia = input.media ?? 'video';
  ...
  const outcomes = await Promise.all(providers.map(async (name) => {
    try {
      ...
      return { name, items: [] as StockItem[], note: `failed — ${error instanceof Error ? error.message : String(error)}`, failed: true };
    }
  }));
