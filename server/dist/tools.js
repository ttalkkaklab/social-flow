...
Returns: categorized text lists — 32 genres, 25 moods, 22 instruments (non-exhaustive).`,
    ...
    // 멀티 채널: channel(브랜드 slug) 지정 시 <SNS_TOKEN_DIR>/<slug>/ 토큰만 사용(폴백 없음).
    {
        ...
        description: `⚠️ Threads 직접 게시 — 로컬 토큰으로 Threads API 에 **즉시 공개 게시**한다(게시 계정은 토큰의 /me 로 자동 결정). 별도 검토 게이트가 없으므로 반드시 사용자가 최종 문안·미디어를 확인·승인한 직후에만 호출한다(HITL — 승인 없이 호출 금지). 이 툴이 지원하는 형태는 텍스트 또는 이미지 1장이다 — 플랫폼 자체는 영상·캐러셀도 지원하지만, 이 파이프라인은 영상을 YouTube·릴스로 내보내고 Threads 에는 커버 이미지 본문 + 풀영상 링크 답글로 유입을 만드는 전략을 쓴다. 본문 링크 금지 플랫폼 규칙: 링크는 본문이 아니라 게시 성공 직후 응답 postId 를 replyToId 로 넣은 답글로 이어서 게시한다. 게시 쿼터는 24시간당 250건. ${SNS_HITL_LINE}`,
        inputSchema: {
            type: 'object',
            properties: {
                caption: {
