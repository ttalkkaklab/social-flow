...
const depthInput = (() => {
    try {
        return storyboardContract().DEPTH_SCHEMA;
    }
    catch {
...
const storyboardVocabulary = (() => {
    try {
        return storyboardContract().VOCAB;
    }
    catch {
        return null;
    }
...
const enumInput = (values, description) => ({ type: 'string', ...(values?.length ? { enum: values } : {}), description });
const purposes = (() => { try {
