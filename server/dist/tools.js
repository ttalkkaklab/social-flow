...
const SNS_CHANNEL_PROPERTY = {
    ...
    description: '채널(브랜드) slug — data/<slug>/ 와 동일한 kebab-case. 지정 시 <SNS_TOKEN_DIR>/<slug>/ 의 토큰만 쓰며 없으면 명시적 에러를 반환한다(기본 토큰 폴백 없음 — 오계정 게시 방지). 생략 시 기본(평면) 토큰 — 채널 디렉토리를 운영 중이면 반드시 지정할 것.',
};
...
const COMMENT_INBOX_OUTPUT = {
    ...
    properties: {
        ...
        summary: {
            type: 'object',
            description: 'postsScanned · commentsFetched · actionable · byPlatform · withinGoldenHour(첫 60분 미응대 수) · oldestActionableMinutes · filters',
        },
...
const CONTENT_FEEDBACK_OUTPUT = {
    ...
    properties: {
        ...
        youtube: {
            type: 'object',
            description: '{ available, error?, account, cohort, items[], notes[] } — items 는 최근 영상별 hook(초반 통과%)·retain(평균 시청%)·angle(조회만 낮고 훅·유지가 산 편)·문제/가설/다음 편',
        },
        instagram: {
            type: 'object',
            description: '{ available, error?, account, cohort, items[], notes[] } — 릴스면 skip(3초 이탈%)·watch(초)·shareRate, 아니면 pending',
        },
    },
...
const YOUTUBE_TOPIC_SCOUT_OUTPUT = {
    ...
    properties: {
        ...
        method: {
            type: 'object',
            description: '{ baseline, minMultiplier, minViews, publishedAfterDays, duration, regionCode, language, via, note } — 채널 중앙값 대비 배수 계약. 기본 시장은 US/en',
        },
        scanned: {
            type: 'object',
        ...
        keywords: {
            type: 'array',
            description: '{ phrase, score, outlierCount, bestMultiplier, evidence[] } — 아웃라이어 제목에서 뽑은 주제어. 점수=배수 합',
            items: { type: 'object' },
        },
        outliers: {
            type: 'array',
            description: '{ videoId, permalink, title, channelTitle, views, baseline, multiplier, publishedAt, durationSeconds, commentCount, tags, gaps[] } — 채널 중앙값 대비 minMultiplier 이상',
            items: { type: 'object' },
        },
        channels: {
            type: 'array',
...
const SNS_ISSUE_SCOUT_OUTPUT = {
    ...
    properties: {
        ...
        method: {
            type: 'object',
            description: '{ via, recency, gl, hl, pagesPerQuery, sites, ranking, scoring, trending } — 구글 site: 검색 계약. ranking 은 관련도순(참여량 아님), scoring 은 언급 글 수 × 플랫폼 가중',
        },
        scanned: {
            type: 'object',
        ...
        keywords: {
            type: 'array',
            description: '{ phrase, score, postCount, platformCount, platforms[], evidence[{platform,url,title}] } — 여러 글·여러 플랫폼에 같이 나온 주제어. 점수는 언급 글 수 기반이라 유튜브 배수와 다른 잣대',
            items: { type: 'object' },
        },
        posts: {
            type: 'array',
            description: '{ platform, url, author, title, snippet, date?, matchedQueries[] } — 게시물 단위(프로필·태그 페이지 제외, 슬러그·미디어 경로 정규화). 참여량 필드는 없다',
            items: { type: 'object' },
        },
        trending: {
            type: 'object',
            description: '{ geo, hours, count, items[{query, searchVolume, increasePct, active, startedAt, categories[], breakdown[], matchesSeed}] } — Google Trends 급상승 검색어. includeTrending=false 면 없음',
        },
    ...
    },
    {
        ...
        inputSchema: {
            ...
            properties: {
                ...
                categoryId: {
                    type: 'number',
                    description: '카테고리로 좁힐 때 (선택). 3=Business and Finance · 4=Entertainment · 7=Health · 14=Politics · 16=Shopping · 17=Sports · 18=Technology · 19=Travel — 없으면 전체',
                },
...
Returns: categorized text lists — 32 genres, 25 moods, 22 instruments (non-exhaustive).`,
    ...
    // 멀티 채널: channel(브랜드 slug) 지정 시 <SNS_TOKEN_DIR>/<slug>/ 토큰만 사용(폴백 없음).
    {
        ...
        description: `⚠️ Threads 직접 게시 — 로컬 토큰으로 Threads API 에 **즉시 공개 게시**한다(게시 계정은 토큰의 /me 로 자동 결정). 별도 검토 게이트가 없으므로 반드시 사용자가 최종 문안·미디어를 확인·승인한 직후에만 호출한다(HITL — 승인 없이 호출 금지). 이 툴이 지원하는 형태는 텍스트(선택적 링크 프리뷰 카드) 또는 이미지 1장이다 — 플랫폼 자체는 영상·캐러셀도 지원하지만, 이 파이프라인은 영상을 YouTube·릴스로 내보내고 Threads 에는 구어체 본문 + 영상 링크(linkUrl)로 유입을 만드는 전략을 쓴다. 영상 회차는 커버 이미지를 붙이지 않는다 — 링크 프리뷰 카드가 그 자리를 대신하고, 한 번의 호출로 게시가 끝난다(링크를 답글로 따로 달지 않는다). 게시 쿼터는 24시간당 250건. ${SNS_HITL_LINE}`,
        inputSchema: {
            type: 'object',
            properties: {
                caption: {
                    type: 'string',
                    description: '게시 본문 완성본 ≤500자 (해시태그는 ≤1개 권장 — 랭킹 가중치 0). 이모지는 플랫폼이 UTF-8 바이트로 세므로 1자보다 크게 계산된다',
                },
                imageUrl: { type: 'string', format: 'uri', description: '공개 접근 가능한 이미지 URL 1장 (플랫폼이 크롤 — 로컬 경로 불가). linkUrl 과 배타' },
                linkUrl: {
                    type: 'string',
                    format: 'uri',
                    description: '본문에 붙일 링크 프리뷰 카드 URL (영상 회차의 릴스·쇼츠 permalink). 텍스트 게시 전용이라 imageUrl 과 배타이며, caption 본문에 같은 URL 을 적어도 플랫폼은 링크 1개로 센다(상한 5개)',
                },
    ...
    },
    {
        ...
        description: `⚠️ Instagram 직접 게시 — 로컬 토큰으로 IG API 에 **즉시 공개 게시**한다(게시 계정은 토큰의 /me 로 자동 결정). 별도 검토 게이트가 없으므로 반드시 사용자가 최종 문안·미디어를 확인·승인한 직후에만 호출한다(HITL — 승인 없이 호출 금지). 이미지 1~10장(2장 이상 캐러셀) **또는** 릴스 영상 1개(imageUrls 와 배타). 이미지는 **JPEG 만** 받는다(PNG·MPO·JPS 거부). 게시 후 이미지 교체 불가(캡션만 수정 가능). 게시 쿼터는 24시간 이동 구간당 100건(캐러셀은 1건). ${SNS_HITL_LINE}`,
        inputSchema: {
            type: 'object',
            properties: {
    ...
    },
    {
        ...
        inputSchema: {
            ...
            properties: {
                ...
                categoryId: {
                    type: 'string',
                    description: 'YouTube 카테고리 id (기본 "22" = People & Blogs). 추천·탐색 분류에 쓰이므로 채널 성격에 맞춰 지정할 것 — 24=Entertainment, 25=News & Politics, 26=Howto & Style, 27=Education, 28=Science & Technology.',
                },
                madeForKids: {
                    type: 'boolean',
    ...
    },
    {
        ...
        inputSchema: {
            ...
            properties: {
                ...
                outputPath: {
                    type: 'string',
                    description: 'HTML 저장 경로. 상대면 cwd 기준. 생략 시 data/<channel>/growth/review-recent.html. .html 만 허용, .. 금지',
                },
            },
        },
    },
    {
        ...
        description: '내 분야에서 이미 터진 유튜브 주제를 찾는다(읽기 전용). 시드 검색어로 관련 채널을 모은 뒤 각 채널의 최근 업로드 조회수 중앙값을 구하고, 그 중앙값의 5배(기본) 이상 나온 영상만 아웃라이어로 본다 — 절대 조회수는 채널 크기마다 기본값이 달라 주제를 고르는 잣대가 못 된다. 아웃라이어 제목에서 주제어를 뽑아 keywords 로 돌려준다. 주제만 참고하고 제목·썸네일·대본은 베끼지 말 것. includeComments=true 면 상위 아웃라이어 댓글에서 미해결 질문(결핍)을 gaps 에 싣는다. 인증은 YOUTUBE_API_KEY 가 우선(게시 쿼터를 안 씀)이고, 없으면 채널 OAuth youtube.readonly. search.list 가 호출당 100유닛이라 시드는 최대 4개.',
        inputSchema: {
            type: 'object',
            properties: {
                query: {
                    type: 'string',
                    description: '시드 검색어 — 채널 profile 주제 영역에서 뽑은 한 줄(예: "AI 업무 자동화"). 검색 툴 공통 인자명',
                },
                extraQueries: {
                    type: 'array',
                    items: { type: 'string' },
