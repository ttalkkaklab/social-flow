...
export const mediaUploadSchema = z.object({
    ...
    if (Number(Boolean(a.shotId)) + Number(a.shotNo !== undefined) !== 1)
        ctx.addIssue({ code: 'custom', message: 'Choose shotId or shotNo' });
    if ((a.kind === 'narration_segment') !== (a.segmentIndex !== undefined))
        ctx.addIssue({ code: 'custom', message: 'segmentIndex is required only for narration_segment' });
});
...
function readMedia(file, kind) {
    ...
    if (!mime || !(['image', 'end_frame'].includes(kind) ? mime.startsWith('image/') : ['narration', 'narration_segment'].includes(kind) ? mime.startsWith('audio/') : mime === 'video/mp4'))
...
export async function uploadShotMedia(args, fetchImpl) {
    ...
    if (args.kind === 'narration_segment') {
        ...
        if (!Array.isArray(segments) || args.segmentIndex >= segments.length)
            throw new Error('segmentIndex does not identify a narration segment.');
    }
    ...
    try {
        ...
        uploaded = data;
        if (!z.string().uuid().safeParse(data.id).success || data.sha256 !== media.sha256 || data.byteSize !== media.bytes.length || data.mime !== media.mime || data.kind !== transportKind)
        ...
        const portalMedia = shots[index].portalMedia;
