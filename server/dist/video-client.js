...
function readVideoHandle(videoPath) {
    try {
        const parsed = JSON.parse(fs.readFileSync(`${videoPath}${HANDLE_SUFFIX}`, 'utf8'));
...
export async function extendVideo(request) {
    ...
    try {
        ...
        if (!sourceVideoUri) {
            return {
                success: false,
                error: `No Veo file handle for ${request.sourceVideoPath} (expected ${request.sourceVideoPath}${HANDLE_SUFFIX}). ` +
