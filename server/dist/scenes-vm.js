...
class LiteralParser {
    ...
    take(text) {
        this.skip();
        if (!this.source.startsWith(text, this.offset))
    ...
    string() {
        ...
        while (this.offset < this.source.length) {
            ...
            if (ch === quote)
                return out;
            if (ch === '\n' || ch === '\r')
                this.fail('a string cannot contain a raw newline');
            if (ch !== '\\') {
                out += ch;
                continue;
            }
    ...
    number() {
        ...
        if (!match)
            this.fail('expected a JSON number');
        this.offset += match[0].length;
    ...
    script() {
        ...
        for (;;) {
            ...
            if (this.identifier() !== 'window')
                this.fail('only window.KEY assignments are allowed');
            this.take('.');
...
export function evaluateWindowScript(source, _options = {}) {
