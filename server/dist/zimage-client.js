...
export const ZIMAGE_QUANTIZE_OPTIONS = [4, 6, 8];
 ...
 */
export function zimageTimeoutMs(width, height, steps) {
...
export async function generateLocalImage(request) {
    const bin = mfluxZImageBin();
    if (!existsSync(bin)) {
        return { success: false, error: installHint(`mflux binary not found: "${bin}"`) };
    }
    const outFile = resolveOutputFile(request.outputPath || process.cwd(), request.filename || `zimage_${Date.now()}.png`, 'image');
    ...
    if (request.seed !== undefined)
        cliArgs.push('--seed', String(request.seed));
    ...
    console.error(`[Z-Image] Generating locally... (${request.width}x${request.height}, ${request.steps} steps, q${request.quantize})`);
    const startedAt = Date.now();
    try {
        await new Promise((resolve, reject) => {
            execFile(bin, cliArgs, {
                timeout: zimageTimeoutMs(request.width, request.height, request.steps) +
