...
export async function webSearch(input) {
    ...
    const slim = compact({
        ...
        organic_results: (p.organic_results ?? []).slice(0, num).map((r) => compact({
            position: r.position,
            title: r.title,
            link: r.link,
            snippet: r.snippet,
...
export async function newsSearch(input) {
    ...
    const slim = compact({
        query: input.q,
        news_results: (p.news_results ?? []).slice(0, max).map((r) => compact({
            position: r.position,
            title: r.title,
...
export async function naverSearch(input) {
    ...
    const slim = compact({
        query: input.query,
        web_results: slimItems(p.web_results),
        news_results: slimItems(p.news_results),
    });
    return { text: JSON.stringify(slim, null, 1), isError: false };
}
