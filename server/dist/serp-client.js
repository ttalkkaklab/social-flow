...
export async function fetchTrendingNow(input) {
    ...
    if (result)
        return result.isError ? { error: result } : { data: { geo, hours, count: 0, received: 0, trends: [] } };
    const p = json;
    const raw = p.trending_searches ?? [];
    const trends = raw.slice(0, limit).map((t) => ({
        query: str(t.query),
        ...
        categories: (t.categories ?? []).map((c) => str(c?.name)).filter(Boolean),
        breakdown: (t.trend_breakdown ?? []).slice(0, TREND_BREAKDOWN_CAP).map(str).filter(Boolean),
    }));
...
export async function trendingNow(input) {
...
export async function fetchGoogleOrganic(input) {
    ...
    if (result)
        return result.isError ? { error: result, hits: [] } : { hits: [] };
    const p = json;
    const offset = input.page && input.page > 1 ? (input.page - 1) * GOOGLE_PAGE_SIZE : 0;
    const hits = (p.organic_results ?? [])
