 ...
 */
export function episodePathArg(args) {
...
export function findEpisodeDir(outputPath) {
    ...
    for (let i = 0; i <= MAX_WALK_UP; i++) {
        try {
            if (existsSync(path.join(dir, EPISODE_MARKER)))
                return dir;
        }
        catch {
            return null;
 ...
 */
export function recordUsage(outputPath, event) {
    ...
    try {
        ...
        console.error(`[social-flow] could not write the usage ledger (${file}): ` +
 ...
 */
export function priceOf(tool, args) {
    if (tool.startsWith('veo_')) {
        ...
        if (!tier)
            return { key: null, quantity: null, note: `unmapped veo model: ${String(args.model)}` };
    ...
    if (tool.startsWith('tts_elevenlabs_')) {
        ...
        return {
            key,
            quantity: null,
            note: 'ElevenLabs bills the metered character-cost header, which sits below the raw text ' +
    ...
    if (tool === 'suno_generate')
        return { key: 'music.suno-generate', quantity: 1 };
    if (tool === 'suno_generate_sound')
        return { key: 'music.suno-sound', quantity: 1 };
    if (tool === 'suno_generate_lyrics')
        return { key: 'music.suno-lyrics', quantity: 1 };
...
function rawChars(args) {
    ...
    if (typeof args.script === 'string')
        return args.script.length;
    if (Array.isArray(args.inputs)) {
        return args.inputs.reduce((sum, t) => sum + (t && typeof t === 'object' && typeof t.text === 'string'
            ? (t.text).length
            : 0), 0);
    }
    return 0;
