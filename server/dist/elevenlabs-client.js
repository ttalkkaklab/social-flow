...
export const ELEVENLABS_SFX_SECONDS = { min: 0.5, max: 30 };
 ...
 */
export const ELEVENLABS_SFX_OUTPUT_FORMATS = ['pcm_48000', 'pcm_44100', 'pcm_24000', 'mp3_44100_128'];
export const DEFAULT_ELEVENLABS_SFX_OUTPUT_FORMAT = 'pcm_48000';
/** Per minute of generated audio, USD, every plan (pricing/api, read 2026-09-16). */
export const ELEVENLABS_SFX_USD_PER_MINUTE = 0.12;
export function sfxExtensionForFormat(format) {
    return format.startsWith('mp3_') ? '.mp3' : '.wav';
}
export function sfxSampleRateForFormat(format) {
...
export function soundEffectRequestBody(request) {
    return {
        text: request.text,
...
export async function generateElevenLabsSoundEffect(request) {
    const outputDir = request.outputPath || process.cwd();
    const ext = sfxExtensionForFormat(request.outputFormat);
    const filename = request.filename || `elevenlabs_sfx_${Date.now()}${ext}`;
    try {
        console.error(`[ElevenLabs] Sound effect... (${request.text.length} chars, ` +
        ...
        const response = await elevenFetch(`/v1/sound-generation?output_format=${request.outputFormat}`, { method: 'POST', body: JSON.stringify(body) }, 120_000);
        const meta = readMeta(response);
