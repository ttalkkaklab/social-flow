...
async function searchOneType(keyword, type, page, perPage) {
    ...
    if (!res.ok)
        return { error: `data.go.kr 검색 HTTP ${res.status}: ${res.body.slice(0, 200)}` };
...
export async function searchDatasets(input) {
    ...
    if (empty) {
        return {
            text: '(검색 결과 없음 — 검색어를 바꿔 재시도할 것. 계속 0건이면 포털 개편으로 파싱이 깨졌을 수 있다: ' +
...
export async function datasetDetail(input) {
    ...
    const res = await requestRaw('get', url, { 'User-Agent': BROWSER_UA }, undefined, 20_000);
    if (!res.ok)
        return err(`data.go.kr 상세 HTTP ${res.status} — publicDataPk(${input.publicDataPk})·type 이 검색 결과와 일치하는지 확인할 것`);
    const html = res.body;
...
export async function downloadFile(input) {
    ...
    if (!step1.ok)
        return err(`다운로드 1단계 HTTP ${step1.status}: ${step1.body.slice(0, 200)}`);
    let ticket;
    try {
    ...
    if (!fileRes.ok)
        return err(`다운로드 2단계 HTTP ${fileRes.status}`);
    ...
    if (buf.length > MAX_DOWNLOAD_BYTES)
        return err('파일이 100MB 상한 초과 — datago_file_fetch 로 필요한 행만 조회할 것.');
