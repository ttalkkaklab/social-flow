...
export const UNIT_TOOLS = UNIT_TOOL_NAMES.map(name => {
    ...
    const keys = ['channel'];
    const required = [];
    const add = (key, mandatory = false) => { keys.push(key); if (mandatory)
        required.push(key); };
    if (unit) {
        add('episodeId', true);
        if (!read) {
        ...
        if (area === 'episode_meta')
            add('key', true);
        if (area.startsWith('shot_'))
            add('shotId', true);
        if (area === 'shot_narration' && !['list', 'create', 'reorder'].includes(action))
            add('index', true);
        if (['shot_narration', 'shot', 'scene', 'sequence'].includes(area) && action === 'create')
            add('index');
        if (area === 'sequence' && action === 'create')
            add('moveScenes');
        if (['sequence', 'shot'].includes(area) && ['get', 'update', 'delete'].includes(action))
            add('id', true);
        if (area === 'scene' && ['get', 'update', 'delete'].includes(action))
            add('no', true);
        ...
        if (area === 'storyboard' && action !== 'create' || ['background', 'prop'].includes(area) || area === 'episode' && action === 'list')
            add('storyboardId', true);
        if (['episode', 'scenario', 'render_allocation', 'episode_audio'].includes(area) && !(area === 'episode' && action === 'list'))
            add('episodeId', true);
        if (['background', 'prop'].includes(area) && !['list', 'create'].includes(action))
            add('id', true);
    ...
    return {
        ...
        inputSchema: { type: 'object', properties: Object.fromEntries(keys.map(key => [key, described[key]])), required, additionalProperties: false },
    };
});
