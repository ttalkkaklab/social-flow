...
export const SUNO_MODELS = ['V4', 'V4_5', 'V4_5PLUS', 'V4_5ALL', 'V5', 'V5_5'];
export const DEFAULT_SUNO_MODEL = 'V5';
/** Sounds tasks accept V5 only (vendor constraint). */
export const SUNO_SOUND_MODEL = 'V5';
export const SUNO_SOUND_KEYS = [
    'Any', 'Cm', 'C#m', 'Dm', 'D#m', 'Em', 'Fm', 'F#m', 'Gm', 'G#m', 'Am', 'A#m', 'Bm',
    'C', 'C#', 'D', 'D#', 'E', 'F', 'F#', 'G', 'G#', 'A', 'A#', 'B',
];
export const SUNO_VOCAL_GENDERS = ['m', 'f'];
export const SUNO_PERSONA_MODELS = ['style_persona', 'voice_persona'];
    ...
    switch (code) {
        ...
        default:
            if (httpStatus && httpStatus >= 400)
                return `HTTP ${httpStatus} (code ${code ?? '?'}): ${text}`;
...
function tracksFromRecord(info) {
    ...
    if (!Array.isArray(list))
        return [];
    return list.map((item, index) => parseTrack(item, index)).filter((t) => t !== undefined);
}
...
function transcodeToWav(src, dest) {
    const result = spawnSync('ffmpeg', ['-y', '-v', 'error', '-i', src, '-acodec', 'pcm_s16le', '-ar', '48000', '-ac', '2', dest], { encoding: 'utf8', timeout: 120_000 });
    if (result.error) {
        throw new Error(`ffmpeg failed to start (${result.error.message}). This plugin requires ffmpeg — brew install ffmpeg.`);
    }
    if (result.status !== 0) {
...
async function saveTracks(parsed, outputDir, filename, pickTrack) {
    if (parsed.length === 0)
        throw new Error('Task succeeded but no audio URL in response');
...
function isDone(status, tracksReady) {
    if (!status)
        return false;
    if (DONE_STATUSES.has(status) && tracksReady)
        return true;
    // Generate finished, callback failed — treat as success when audio is present.
    if (status === 'CALLBACK_EXCEPTION' && tracksReady)
        return true;
    return false;
}
async function pollRecord(taskId, opts) {
    ...
    for (let poll = 1; poll <= opts.maxPolls; poll += 1) {
        ...
        if (isDone(status, tracks.length > 0))
            return last;
    ...
    throw new Error(`${opts.label} timed out (${(opts.maxPolls * opts.intervalMs) / 60_000} minutes). ` +
...
async function pollLyrics(taskId) {
    ...
    for (let poll = 1; poll <= LYRICS_MAX_POLLS; poll += 1) {
        ...
        if (status === 'SUCCESS' && lyrics.length > 0)
            return last;
        if (status === 'CALLBACK_EXCEPTION' && lyrics.length > 0)
            return last;
    }
    throw new Error(`Lyrics timed out (${(LYRICS_MAX_POLLS * LYRICS_POLL_INTERVAL_MS) / 60_000} minutes). Task ${taskId}`);
}
// ── public API ─────────────────────────────────────────────────
function assertGenerateArgs(request) {
    ...
    if (!request.style)
        throw new Error('style is required when customMode is true');
    if (!request.title)
        throw new Error('title is required when customMode is true');
...
export async function generateMusic(request) {
    try {
        ...
        if (request.personaModel)
            body.personaModel = request.personaModel;
        ...
        const parsed = tracksFromRecord(info).filter((t) => t !== undefined);
        const tracks = await saveTracks(parsed, request.outputPath || process.cwd(), request.filename, request.pickTrack ?? 0);
 ...
 * with `-stream_loop -1`.
 */
export async function generateSound(request) {
    try {
        const body = {
        ...
        if (request.soundTempo !== undefined)
            body.soundTempo = request.soundTempo;
        if (request.soundKey)
            body.soundKey = request.soundKey;
...
export async function generateLyrics(request) {
    try {
        ...
        if (lyrics.length === 0)
            return { success: false, error: 'Lyrics task succeeded but returned no text', taskId };
...
export async function getCredits() {
    try {
        ...
        const credits = typeof envelope.data === 'number'
            ? envelope.data
