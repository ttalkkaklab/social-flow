...
export function watchSeconds(msOrSec) {
    if (msOrSec == null || !Number.isFinite(msOrSec) || msOrSec < 0)
        return null;
    return msOrSec > 200 ? msOrSec / 1000 : msOrSec;
}
function vsMedian(value, mid, higherIsBetter) {
    if (value == null || mid == null || mid === 0)
        return 'na';
...
function worstTone(steps) {
    if (steps.some((s) => s.lever === 'pending'))
        return 'pending';
    if (steps.length === 0)
        return 'ok';
...
export function analyzeYoutubeVideos(videos, account, channelMetrics) {
    const notes = [
    ...
    const cohort = {
        ...
        channelSubRate: (() => {
            ...
            if (gained == null || views == null || views <= 0)
                return null;
    ...
    if (account && account.subscriberCountHidden) {
...
export function analyzeInstagramMedia(media, limit) {
    const notes = [
    ...
    const cohort = {
        skip: median(reelRows.map((r) => r.skip).filter((n) => n != null)),
        watch: median(reelRows.map((r) => r.watch).filter((n) => n != null)),
        shareRate: median(reelRows.map((r) => r.shareRate).filter((n) => n != null)),
        reach: median(reelRows.map((r) => r.reach).filter((n) => n != null)),
    };
    const items = rows.map(({ item, metrics, skip, watch, reach, shares, shareRate }) => {
...
function resolveHtmlPath(channel, outputPath) {
    ...
    if (channel && CHANNEL_SLUG_RE.test(channel))
        return defaultHtmlPath(channel);
    return null;
}
export async function contentFeedback(input) {
...
function sectionFromYoutube(res, _limit) {
    if (!res.ok) {
        return {
            platform: 'YOUTUBE',
            available: false,
    ...
    if (typeof parsed?.videosError === 'string')
        analyzed.notes.push(String(parsed.videosError));
    return {
        platform: 'YOUTUBE',
        available: true,
        account,
...
function sectionFromInstagram(res, limit) {
    if (!res.ok) {
        return {
            platform: 'INSTAGRAM',
            available: false,
    ...
    if (profileViews != null)
        analyzed.cohort.profileViews = profileViews;
    return {
        platform: 'INSTAGRAM',
        available: true,
        account,
