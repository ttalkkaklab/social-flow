...
function kindOf(name, root) {
    ...
    if (root === 'storyboard/.portal-local' && stamp) {
        ...
        if (Number.isFinite(time) && new Date(time).toISOString() === iso)
            return { kind: stamp[2] === 'scenarios' ? 'scenarios' : 'board', time };
    }
    if (root === 'storyboard/.portal-local' && new RegExp(`^images-${uuid}$`).test(name))
        return { kind: 'images' };
    if (root === '.portal-local' && new RegExp(`^attachments-${uuid}$`).test(name))
...
export function manageBackups(input) {
    ...
    const entries = [], ignored = [], fingerprint = [];
    let visited = 0;
    function scan(relative) {
        if (++visited > 100000)
            throw new Error('Backup inventory exceeds 100000 entries; no backups were pruned.');
        const stat = lstatSync(path.join(dir, relative));
        if (stat.isSymbolicLink() || (!stat.isDirectory() && !stat.isFile()))
            throw new Error(`Unsafe backup entry: ${relative}`);
    ...
    for (const root of ['storyboard/.portal-local', '.portal-local']) {
        if (root.startsWith('storyboard/'))
            safeAttachmentTarget(dir, 'storyboard/backup-root-check');
        const rootStat = statIfPresent(path.join(dir, root));
        if (!rootStat)
            continue;
        if (rootStat.isSymbolicLink() || !rootStat.isDirectory())
            throw new Error(`Unsafe backup root: ${root}`);
        ...
        for (const name of readdirSync(path.join(dir, root)).sort()) {
            ...
            if (!stat.isDirectory() || stat.isSymbolicLink())
                throw new Error(`Unsafe backup directory: ${relative}`);
    ...
    const remove = entries.filter(entry => {
        const n = (counts.get(entry.kind) ?? 0) + 1;
    ...
    if (args.apply && args.confirm !== plan)
        throw new Error('Backup inventory changed or confirmation does not match. Preview again; nothing was pruned.');
    const deleted = [];
    let error;
    if (args.apply) {
        try {
            for (const entry of remove) {
                rmSync(path.join(dir, entry.path), { recursive: true });
                deleted.push(entry.path);
            }
        }
        catch (cause) {
            error = `Pruning stopped: ${cause instanceof Error ? cause.message : String(cause)}`;
        }
    }
