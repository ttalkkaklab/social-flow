...
function parseArgs(schema, args) {
    ...
    if (known && args && typeof args === 'object' && !Array.isArray(args)) {
        ...
        if (unknown.length > 0) {
            throw new Error(`Invalid arguments — unknown argument(s): ${unknown.join(', ')}. ` +
...
const naverSearchSchema = z.object({
    ...
    type: z
        ...
        errorMap: () => ({
            message: `Available: ${naver.NAVER_SEARCH_TYPES.join(' | ')}. ` +
    ...
    sort: z.enum(naver.NAVER_SORTS).optional(),
...
const facebookPublishSchema = z
    ...
    if (v.captionFilePath && !v.videoUrl)
...
const ytDescription = z
    ...
    message: `The description is ${Buffer.byteLength(v, 'utf8')} bytes — YouTube's cap is ${YT_DESCRIPTION_MAX_BYTES} bytes (not characters). ` +
...
const REPLY_MAX_CHARS = { THREADS: 500, INSTAGRAM: 2200, FACEBOOK: 8000, YOUTUBE: 10_000 };
...
export const ROUTES = {
    ...
    image_local_generate: async (args) => {
        ...
        if (!result.success)
            return text(`Local image generation failed: ${result.error}`, true);
    ...
    tts_local_generate: async (args) => {
        ...
        if (!result.success)
            return text(`Local TTS generation failed: ${result.error}`, true);
        return text(`Audio generated locally!\n\nFile: ${result.audioPath}\nEngine: Supertonic 3 (on-device)\n` +
    ...
    tts_list_voices: async () => {
        const voiceList = Object.entries(tts.TTS_VOICES)
            ...
            .join('\n');
        return text(`Gemini TTS — ${tts.TTS_VOICE_NAMES.length} voices (tts_generate / tts_multi_speaker):\n\n${voiceList}\n\n` +
    ...
    blender_scene_read: async (args) => {
        ...
        if (!r.success)
            return text(`Blender scene read failed: ${r.error}`, true);
    ...
    blender_scene_build: async (args) => {
        ...
        if (!r.success)
            return text(`Blender scene build failed: ${r.error}`, true);
        const built = (r.scene.built ?? []).map((b) => (b.kind ? `${b.name} (${b.kind})` : `${b.name} (glb, ${b.objects} objects)`));
        return text(`Blender previz set ${request.reset ? 'built' : 'extended'} — ${built.length} added${built.length ? `: ${built.join(', ')}` : ''}.\n` +
    ...
    blender_camera_set: async (args) => {
        ...
        if (!r.success)
            return text(`Blender camera set failed: ${r.error}`, true);
        const keys = r.scene.applied?.keyframes ?? [];
        return text(`Camera "${r.scene.applied?.camera ?? 'Camera'}" ${keys.length ? `keyed at frames [${keys.join(', ')}]` : 'set as a static shot'}.\n` +
    ...
    blender_object_animate: async (args) => {
        ...
        if (!r.success)
            return text(`Blender object animate failed: ${r.error}`, true);
        const keys = r.scene.applied?.keyframes ?? [];
    ...
    suno_generate: async (args) => {
        ...
        if (!result.success)
            return text(`Suno generation failed: ${result.error}`, true);
        ...
        return text(`Suno tracks generated.\n\nPrimary: ${result.audioPath}\nModel: ${result.model}\nTask: ${result.taskId}\nTracks (${tracks.length}):\n${lines.join('\n')}\n\n` +
    ...
    suno_generate_sound: async (args) => {
        ...
        if (!result.success)
            return text(`Suno sound generation failed: ${result.error}`, true);
        const tracks = result.tracks ?? [];
        const lines = tracks.map((track, index) => `  ${index + 1}. ${track.audioPath}`);
        return text(`Suno sound generated.\n\nFile: ${result.audioPath}\nModel: ${result.model}\nTask: ${result.taskId}\nTracks:\n${lines.join('\n')}\n\n` +
    ...
    suno_generate_lyrics: async (args) => {
        ...
        if (!result.success)
            return text(`Suno lyrics generation failed: ${result.error}`, true);
        const blocks = (result.lyrics ?? []).map((item, index) => {
            const heading = item.title ? `Variant ${index + 1} — ${item.title}` : `Variant ${index + 1}`;
            return `${heading}\n${item.text}`;
        });
        return text(`Suno lyrics generated (${result.lyrics?.length ?? 0} variants). Task: ${result.taskId}\n\n${blocks.join('\n\n---\n\n')}\n\n` +
    ...
    suno_credits: async () => {
        ...
        if (!result.success)
            return text(`Suno credits lookup failed: ${result.error}`, true);
        return text(`Suno remaining credits: ${result.credits}\n\n` +
    ...
    threads_publish: async (args) => {
        ...
        if (input.draftId)
            checkThreadsGate(input);
        else
            checkThreadsEpisode(input);
        if (input.dryRun)
            return fromApi({ ok: true, status: 200, body: JSON.stringify({ platform: 'THREADS', postId: '', dryRun: true, gatePassed: true }) }, 'Dry run: no publishing API called.');
    ...
    portal_character_get: async (args) => fromPortal(await portalRoutes.characterGet(parseArgs(portal.characterGetSchema, args))),
