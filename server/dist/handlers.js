...
function parseArgs(schema, args) {
    ...
    if (known && args && typeof args === 'object' && !Array.isArray(args)) {
        ...
        if (unknown.length > 0) {
            throw new Error(`Invalid arguments — 알 수 없는 인자: ${unknown.join(', ')}. ` +
...
const naverSearchSchema = z.object({
    ...
    type: z
        ...
        errorMap: () => ({
            message: `사용 가능: ${naver.NAVER_SEARCH_TYPES.join(' | ')}. ` +
...
const ytDescription = z
    ...
    message: `설명이 ${Buffer.byteLength(v, 'utf8')}바이트다 — YouTube 상한은 ${YT_DESCRIPTION_MAX_BYTES}바이트다(글자 수가 아니다). ` +
