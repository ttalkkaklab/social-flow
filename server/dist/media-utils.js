...
export function validateFilePath(filePath, options) {
    ...
    if (options?.allowedExtensions) {
        ...
        if (!options.allowedExtensions.includes(ext)) {
            throw new Error(`File extension "${ext}" is not allowed. Allowed: ${options.allowedExtensions.join(', ')}`);
        }
    }
}
/** 에러 메시지를 추출한다. */
 ...
 */
export function pcmToWav(pcmData, sampleRate, numChannels) {
 ...
 */
export function resolveOutputFile(outputDir, filename, kind) {
...
export function saveMediaFile(outputDir, filename, data, kind) {
    const fullPath = resolveOutputFile(outputDir, filename, kind);
    fs.writeFileSync(fullPath, data);
    return fullPath;
}
/** 오디오 전용 단축 — TTS·음악 클라이언트가 쓴다. */
export function saveAudioFile(outputDir, filename, data) {
...
export function mimeFromExtension(filePath, kind) {
