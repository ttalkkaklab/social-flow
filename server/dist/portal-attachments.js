...
export function safeAttachmentTarget(root, relative) {
    ...
    for (const part of target.slice(current.length).split(path.sep)) {
        ...
        try {
            if (lstatSync(current).isSymbolicLink())
                throw new Error(`Symlink is not an attachment target: ${relative}`);
        }
        catch (error) {
...
function readBounded(root, relative) {
    ...
    try {
        ...
        if (!stat.isFile() || stat.size > LIMIT)
            throw new Error(`Not a regular file of at most 10 MiB: ${relative}`);
        const bytes = Buffer.alloc(LIMIT + 1);
        let size = 0, count = 0;
        while ((count = readSync(fd, bytes, size, bytes.length - size, null)) > 0) {
            size += count;
            if (size > LIMIT)
                throw new Error(`File grew past 10 MiB: ${relative}`);
        }
...
export async function uploadAttachments(client, episodeId, root) {
    ...
    function walk(relative = '') {
        for (const item of readdirSync(path.join(root, relative), { withFileTypes: true })) {
            ...
            try {
                ...
                else if (lstatSync(target).size > LIMIT)
                    throw new Error('Exceeds 10 MiB');
                else
                    files.push(name);
            }
            catch (error) {
                skipped.push({ path: name, reason: String(error) });
            }
        }
    }
    ...
    if (files.length > 10000)
        throw new Error('Too many episode attachments');
    const metadataFile = path.join(root, MANIFEST);
    if (existsSync(metadataFile) && (lstatSync(metadataFile).isSymbolicLink() || lstatSync(metadataFile).size > LIMIT))
        throw new Error('Unsafe attachment metadata file');
    const local = existsSync(metadataFile) ? JSON.parse(readFileSync(metadataFile, 'utf8')) : {};
    ...
    for (const relative of files.sort()) {
        ...
        if (existing?.sha256 === sha256 && (!provenance || JSON.stringify(existing.provenance) === JSON.stringify(provenance))) {
            unchanged++;
            continue;
        }
        const { data: saved } = await client.uploadAttachment(episodeId, relative, bytes, mime(relative), provenance);
        if (saved.sha256 !== sha256 || saved.byteSize !== bytes.length || saved.relativePath !== relative)
...
export async function restoreAttachments(client, episodeId, root) {
    const { data } = await client.listAttachments(episodeId);
    ...
    for (const item of data.items) {
        ...
        if (bytes.length !== item.byteSize || hash(bytes) !== item.sha256)
    ...
    for (const { item, bytes } of staged) {
        ...
        if (existsSync(target) && !readBounded(root, item.relativePath).equals(Buffer.from(bytes))) {
            ...
            if (existsSync(path.join(root, '.portal-local')) && lstatSync(path.join(root, '.portal-local')).isSymbolicLink())
...
export async function attachmentSyncReport(action) {
    try {
        ...
        return { complete: false, error: error instanceof Error ? error.message : String(error), next: 'Keep the local episode. The board operation already completed; attachment backup is incomplete. Retry portal_attachments_sync after fixing the error.' };
    }
}
