...
export const QWEN3_ASR_MODELS = ['Qwen/Qwen3-ASR-1.7B', 'Qwen/Qwen3-ASR-0.6B'];
/** Korean quality is why this tool exists, so 1.7B is the default. 0.6B only when speed matters. */
export const DEFAULT_QWEN3_ASR_MODEL = 'Qwen/Qwen3-ASR-1.7B';
/**
...
export const DEFAULT_QWEN3_ASR_LANGUAGE = 'Korean';
...
export function canonicalizeLanguage(value) {
    ...
    if (!mapped) {
        throw new Error(`Unsupported language "${value}". Use one of: ${QWEN3_ASR_LANGUAGES.join(', ')} (aliases like ko/en/ja also work).`);
    }
    return mapped;
}
...
export const qwen3AsrTranscribeSchema = z.object({
    audioPath: z
        ...
        .refine((p) => AUDIO_INPUT_EXTENSIONS.includes(extname(p).toLowerCase()), {
...
function probeDurationSeconds(audioPath) {
    return new Promise((resolve) => {
        execFile('ffprobe', ['-v', 'error', '-show_entries', 'format=duration', '-of', 'csv=p=0', audioPath], { timeout: 15_000 }, (error, out) => {
...
export function toWhisperTranscript(data) {
...
// ── Transcription ───────────────────────────────────────────────
export async function transcribeLocal(request) {
    ...
    const outFile = resolveOutputFile(request.outputPath || process.cwd(), request.filename || `stt_${Date.now()}.json`, 'json');
    ...
    if (firstCall) {
        console.error('[Qwen3-ASR] First call — downloading ~3.4GB of weights (plus ForcedAligner if timestamps) to the huggingface cache. This can take a while.');
    }
    ...
    if (request.context?.trim())
        cliArgs.push('--context', request.context.trim());
    console.error(`[Qwen3-ASR] Transcribing locally... (${request.model}, ${request.language}, ${duration.toFixed(1)}s audio)`);
