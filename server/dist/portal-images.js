...
import { z } from 'zod';
import { describePortalError } from './portal-client.js';
...
function readImage(file) {
    ...
    try {
        ...
        if (!stat.isFile() || stat.size === 0 || stat.size > MAX_BYTES)
            throw new Error('Image must be a regular nonempty file of at most 5 MiB.');
        ...
        if (!size || size > MAX_BYTES)
            throw new Error('Image changed size or exceeds 5 MiB.');
        ...
        if (!mime || expected[extension] !== mime)
            throw new Error('Image must be PNG, JPEG or WebP with a matching extension and signature. The portal also validates decoding.');
...
export async function uploadEpisodeImages(client, args, base) {
    ...
    if (ids.some(id => typeof id !== 'string' || !id) || new Set(ids).size !== ids.length)
        throw new Error('Shot IDs must be unique nonempty strings. Nothing was sent.');
    const skipped = [];
    const inputs = args.images ?? shots.flatMap((shot, index) => {
        ...
        if (files.length > 1)
            throw new Error(`Multiple images for shot ${index + 1}; choose one with images[].file.`);
        if (!files.length) {
    ...
    if (inputs.length > 500)
        throw new Error('At most 500 images per call. Nothing was sent.');
    if (!inputs.length)
        throw new Error('No shot images found in storyboard/images/scene-N.{png,jpg,jpeg,webp}. Nothing was sent.');
    ...
    const plan = inputs.map(input => {
        ...
        if (index < 0 || index >= shots.length || seen.has(index))
            throw new Error('Unknown or duplicate shot target. Nothing was sent.');
        if (!input.shotId && shots[index].id !== undefined)
            throw new Error('Use shotId for shots that have an ID. Nothing was sent.');
        seen.add(index);
        const target = realpathSync(path.resolve(sb, input.file));
        if (!target.startsWith(root + path.sep))
            throw new Error('Image path must stay inside storyboard/, including symlinks. Nothing was sent.');
    ...
    try {
        if (!unchanged())
            throw new Error('Local board or portal state changed during validation.');
        for (const item of plan) {
            const image = readImage(item.file);
            if (image.sha256 !== item.sha256)
                throw new Error('Image changed during upload; rerun with the intended files.');
        ...
        if (!unchanged())
            throw new Error('Local board or portal state changed; uploaded blobs are not linked.');
        ...
        phase = 'local';
        if (!unchanged())
            throw new Error('Portal revision saved, but local board or portal state changed. Local files were preserved.');
        const temporary = `${file}.${randomUUID()}.tmp`;
        try {
            writeFileSync(temporary, nextSource);
