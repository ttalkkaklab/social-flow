...
const normalize = (s) => s.normalize('NFC').replace(/\r\n?/g, '\n').trim();
export function threadsBodyHash(body, selfReply = '') {
...
function load(id) {
    const value = JSON.parse(readFileSync(draftLocation(id).file, 'utf8'));
...
export function gateCall(tool, args, action) {
    ...
    if (typeof raw.draftId === 'string') {
        ...
        catch { /* audit invalid id in the fallback directory */ }
    }
    try {
        const result = action();
...
export function createThreadsDraft(args) {
    return gateCall('threads_draft_create', args, () => {
        const input = draftSchema.strict().parse(args);
        if ((input.surface === 'reply') !== Boolean(input.replyToId))
            throw new Error('Reply drafts require replyToId; post drafts must omit it');
        if (!whitespace(input.body).includes(whitespace(input.purposeEvidence)))
            throw new Error('Purpose evidence quote not present in draft body');
...
export function submitThreadsReview(args) {
    return gateCall('threads_review_submit', args, () => {
        const input = reviewSchema.strict().parse(args);
        const draft = load(input.draftId);
        if (input.bodyHash !== draft.bodyHash)
            throw new Error('Review body hash mismatch');
        if (draft.surface === 'reply' && input.axis !== 'voice')
            throw new Error('Reply drafts use voice review only');
        const surfaces = (input.axis !== 'voice' ? [draft.body] : [draft.body, draft.selfReply ?? '']).map(whitespace);
        for (const finding of input.findings) {
            if (!surfaces.some((body) => body.includes(whitespace(finding.quote))))
                throw new Error('Finding quote not present in draft');
        }
...
export function checkThreadsGate(input) {
    return gateCall('threads_publish', input, () => {
        if (!input.draftId)
            throw new Error('Growth publishing requires draftId');
        const draft = load(input.draftId);
        if (input.channel !== draft.channel)
            throw new Error('Draft channel mismatch');
        if (input.replyToId !== draft.replyToId)
            throw new Error('Draft reply target mismatch');
        const bodyHash = threadsBodyHash(input.caption, input.selfReply);
        if (bodyHash !== draft.bodyHash)
            throw new Error('Publish body hash mismatch');
        let limits;
        try {
            ...
            if (error.code === 'ENOENT')
                throw new Error('Gate thresholds not configured (기준점 미설정): gate.json required');
            throw error;
        }
        ...
        for (const axis of axes) {
            ...
            if (!review)
                throw new Error(`Missing ${axis} review`);
            reviewSchema.parse(review);
            if (review.draftId !== draft.draftId || review.axis !== axis)
                throw new Error('Review identity mismatch');
            if (review.findings.some((finding) => finding.severity === 'P0'))
                throw new Error(`${axis} review has unresolved P0`);
            if (review.bodyHash !== bodyHash)
                throw new Error(`${axis} review hash mismatch`);
            if (review.score < limits[axis])
                throw new Error(`${axis} score ${review.score} below ${limits[axis]}`);
        }
...
export function checkThreadsEpisode(input) {
    gateCall('threads_publish', input, () => {
        ...
        if (dirname(episode) !== root || !statSync(episode).isDirectory())
            throw new Error('Invalid episode directory');
        const board = realpathSync(join(episode, 'storyboard', 'scenes.js'));
        if (!board.startsWith(episode + '/') || !statSync(board).isFile())
            throw new Error('Missing episode storyboard');
        const approvalPath = realpathSync(join(episode, 'threads-publish-approval.json'));
        if (dirname(approvalPath) !== episode)
            throw new Error('Invalid episode approval path');
        const approval = JSON.parse(readFileSync(approvalPath, 'utf8'));
        if (approval.approved !== true)
            throw new Error('Episode publishing approval required');
        for (const key of ['caption', 'selfReply', 'imageUrl', 'videoUrl', 'linkUrl', 'replyToId']) {
            if ((approval[key] ?? '') !== (input[key] ?? ''))
                throw new Error(`Episode approval mismatch: ${key}`);
        }
    });
}
