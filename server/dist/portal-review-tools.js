...
export const REVIEW_TOOLS = REVIEW_TOOL_NAMES.map(name => {
    ...
    return { name, title: name.slice(7).replaceAll('_', ' '),
        ...
        inputSchema: { type: 'object', properties: { ...common,
            ...
            }, required: ['episodeId', ...(record ? ['baseRevisionNo', payload] : [])], additionalProperties: false },
    };
});
export async function runReviewTool(name, args, fetchImpl) {
    try {
        const parsed = reviewToolSchemas[name].parse(args);
        const client = await portalClientFor(parsed.channel, fetchImpl);
        if (!client)
            return { content: [{ type: 'text', text: 'Portal API key is not configured for this channel.' }], isError: true };
...
export const REVIEW_ROUTES = Object.fromEntries(REVIEW_TOOL_NAMES.map(name => [name, (args) => runReviewTool(name, args)]));
