...
import { z } from 'zod';
import { applyPatch, contract, transitionPatchSchema } from './storyboard.js';
...
export const META_KEYS = ['THEME', 'COMPREHENSION', 'STORY', 'PRODUCTION', 'MOTION_POLICY', 'STRUCTURE', 'PREVIZ', 'SLIDE_OBJECTS', 'MUSIC', 'VOICE'];
export function editUnit(input, area, action, args) {
    const board = structuredClone(input);
    if (args.globals) {
        if (['get', 'list'].includes(action))
            throw new Error('Read operations do not accept globals');
        for (const [key, value] of Object.entries(args.globals)) {
            if (!META_KEYS.includes(key) || key === 'STRUCTURE')
                throw new Error('Unknown companion meta key');
    ...
    let value;
    const needValue = () => { if (!args.value)
        throw new Error('value is required'); return args.value; };
    const requireItem = (item) => { if (item === undefined)
        throw new Error('Unit not found'); return item; };
    if (area.startsWith('episode_')) {
        ...
        if (!read) {
            board[key] = merge(value, needValue());
            value = board[key];
        }
    }
    else if (area === 'sequence' || area === 'scene' || area === 'shot') {
        let list = area === 'sequence' ? st.sequences : area === 'scene' ? st.scenes : shots;
        const key = area === 'scene' ? 'no' : 'id';
        const id = area === 'scene' ? args.no : args.id;
        if (['get', 'update', 'delete'].includes(action) && id === undefined)
            throw new Error(`${key} is required`);
        const index = list.findIndex(item => item[key] === id);
        if (action === 'list')
            value = list;
        else if (action === 'get')
            value = requireItem(list[index]);
        else if (action === 'reorder')
            list = ordered(list, args.order, item => item[key]);
        else if (action === 'create') {
            const item = needValue();
            if (list.some(v => v[key] === item[key]) && item[key] !== undefined)
                throw new Error('Duplicate identifier');
            if (area !== 'shot' && item[key] === undefined)
                throw new Error(`${key} is required`);
            const at = args.index ?? list.length;
            if (at > list.length)
                throw new Error('index past last unit');
            if (area === 'sequence' && args.moveScenes) {
                if (!Array.isArray(item.scenes))
                    throw new Error('sequence.scenes must be an array');
                for (const sequence of st.sequences)
                    sequence.scenes = sequence.scenes.filter(no => !item.scenes.includes(no));
            }
            ...
            if (area === 'scene') {
                ...
                for (const id of args.shotIds ?? [])
                    requireItem(shots.find(shot => shot.id === id)).scene = item.no;
            }
            value = item;
        }
            ...
            if (patch[key] !== undefined && patch[key] !== old[key])
                throw new Error('Identifiers cannot be renamed');
            list[index] = merge(old, patch);
            value = list[index];
        }
            ...
            if (area === 'sequence' && args.targetSequenceId) {
                if (args.targetSequenceId === id)
                    throw new Error('Cannot transfer to the deleted sequence');
                requireItem(st.sequences.find(q => q.id === args.targetSequenceId)).scenes.push(...drop);
            }
            if (area === 'scene' && args.targetSceneNo) {
                if (args.targetSceneNo === id)
                    throw new Error('Cannot transfer to the deleted scene');
                requireItem(st.scenes.find(scene => scene.no === args.targetSceneNo));
                for (const shot of shots)
                    if (drop.includes(shot.scene))
                        shot.scene = args.targetSceneNo;
            }
            if (!transfer && !args.cascade && (area === 'sequence' && drop.length || area === 'scene' && shots.some(s => drop.includes(s.scene))))
                throw new Error('Children exist; cascade:true is required');
        ...
        if (!read) {
            ...
            if (area === 'shot')
                shots = list;
            if (args.scenes)
                st.scenes.push(...args.scenes);
            if (args.shots)
                shots.push(...args.shots);
        ...
        if (area === 'shot_narration') {
            ...
            if (action === 'list')
                ...
                value = requireItem(lines[args.index]);
            else if (action === 'create') {
                const line = needValue();
                const at = args.index ?? lines.length;
                if (at > lines.length)
                    throw new Error('index past last narration line');
                lines.splice(at, 0, line);
                value = line;
            }
            else if (action === 'reorder')
                lines = ordered(lines.map((line, index) => ({ line, index })), args.order, item => item.index).map(item => item.line);
            else {
                const old = requireItem(lines[args.index]);
                if (action === 'delete')
                    lines.splice(args.index, 1);
                else
                    lines[args.index] = merge(old, needValue());
                value = lines[args.index] ?? null;
            }
            if (!read)
                shot.narration = lines;
        }
            ...
            if (!read) {
                ...
                if (applied.findings.some(f => f.level === 'bad'))
                    throw new Error(JSON.stringify(applied.findings));
            ...
            if (!read) {
                ...
                for (const key of Object.keys(patch))
                    if (!['sound', 'effect'].includes(key))
                        throw new Error('Only sound and effect are accepted');
                Object.assign(shot, merge(shot, patch));
                value = { sound: shot.sound, effect: shot.effect };
            }
        }
        else {
            ...
            if (!key)
                throw new Error('Unknown shot field');
            shot.visual ??= {};
            value = shot.visual[key] ?? null;
            if (!read) {
                const patch = needValue();
                if (key === 'bgPrompt') {
                    if (typeof patch.bgPrompt !== 'string')
                        throw new Error('value.bgPrompt must be a string');
                    shot.visual.bgPrompt = patch.bgPrompt;
                }
                else
                    shot.visual[key] = merge(value, patch);
    ...
    if (!read) {
        ...
        for (const shot of board.SCENES ?? []) {
            ...
            if (old && line && typeof line.matchShot === 'number') {
                ...
                if (at < 0)
                    throw new Error('Removed eyeline target; repair the relation first');
                line.matchShot = at + 1;
            }
        }
        if (board.STRUCTURE)
